...
export function initDME() {
  ...
  function switchTab(newTab) {
    ...
    if (newTab === 'tables' || newTab === 'columns') {
      // The left pane no longer swaps with the tab. Tables and columns sit
      // together in one pane now (a table row expands to its own columns), so
      // Builder and Semantics show the identical Tables pane — including when
      // nothing has been added yet, which used to hide the pane on Semantics.
      // The separate #pane-columns-section has been removed from the JSX.
  ...
  function rebuildTablesCanvas() {
    ...
    // Keep the column tree data (full source schema per table) in sync
    // wherever the canvas itself is rebuilt — the preview panel's column
    // selector needs the full schema, not just added columns, and shouldn't
    // have to wait for the Columns tab to have been opened at least once.
    rebuildColumnPane();
    if (!tables.length) {
  ...
  function rebuildColumnsContent() {
    ...
    if (!hasColumns) {
      ...
      // Used to re-show the old separate columns pane (and hide the whole
      // left pane when nothing was added) while Semantics was active. That
      // pane is gone and the left pane is tab-independent now — see
      // switchTab — so this only paints the main content's empty state.
      if (hasTables) {
        emptyEl.innerHTML = `
...
Only include the context field when there is genuinely meaningful content from the conversation. Set it to null if the user has only sent generic commands (like "add to model") with no domain context. Update it cumulatively — include all context captured so far, not just from the latest message.`,
  ...
  // Exposed for the Optimized sub-header's own Save changes button (React-rendered,
  // outside this legacy-DOM section), so it reuses the same toast.
  ...
  // Join created from the canvas (join handle → Edit Join dialog). Mirrors
  // addTableManually: mutate the model, then rebuild so the line is drawn.
  ...
  // Mirror of addColumnManually, for the data-source-selector Option 2
  // checkbox column list (unchecking removes the column from the model).
  function removeColumnManually(tableName, colName) {
    const state = window._modelState;
    const group = state.model.columns.find(g => g.table === tableName);
    if (!group) return;
    group.columns = group.columns.filter(c => c !== colName);
    state.model.columns = state.model.columns.filter(g => g.columns.length > 0);
    state.addedColumns = state.addedColumns.filter(c => c !== colName);
    window._pendingManualChanges = true;
    rebuildColumnPane();
    rebuildColumnsContent();
  }

  // Bridge for selector Option 2's "+" control — adds a table to the canvas
  // without a drop position, so addTableManually's own findEmptySlot places it.
  window._addTableManually = function(tableName) { addTableManually(tableName); };

  // Bridge for the prototype-only Empty/Demo switch in the left pane's dock.
  // Replaces the whole model in one go rather than replaying addTableManually
  // per table, so the joins land with the cards instead of after them (a join
  // to a table that isn't on the canvas yet draws a line to nothing).
  // tablePositions is cleared so rebuildTablesCanvas re-runs findEmptySlot and
  // lays the demo cards out from the origin, ignoring wherever the previous
  // model's cards had been dragged to.
  window._loadDemoModel = function(demo) {
    const state = window._modelState;
    state.model.tables  = demo.tables.map(t => ({ name: t.name, desc: t.desc || '' }));
    state.model.joins   = demo.joins.map((j, i) => ({ name: j.name || `Join ${i + 1}`, desc: j.desc || '', ...j }));
    state.model.columns = demo.columns.map(g => ({ table: g.table, columns: g.columns.slice() }));
    state.model.formulas = (demo.formulas || []).map(f => ({ name: f.name, code: f.code || f.expression || '' }));
    state.addedTables   = state.model.tables.map(t => t.name);
    state.addedJoins    = state.model.joins.map(j => `${j.leftTable}-${j.rightTable}`);
    state.addedColumns  = state.model.columns.flatMap(g => g.columns.map(c => `${g.table}.${c}`));
  ...
  // The other half of that switch — back to the manually-built starting point
  // (empty canvas). Mirrors the `welcomeVariant !== 'existing'` branch of the
  // initial _modelState above.
  window._resetModel = function() {
    const state = window._modelState;
    state.model.tables   = [];
    state.model.joins    = [];
    state.model.columns  = [];
    state.model.formulas = [];
    state.addedTables    = [];
    state.addedJoins     = [];
    state.addedColumns   = [];
    state.tablePositions = {};
  ...
  // Bridge for selector Option 2's remove control. Takes the table off the
  // canvas along with everything that can't outlive it: its columns, and any
  // join that referenced it (a join to a missing table would draw a line to
  // nothing).
  window._removeTableManually = function(tableName) {
    const state = window._modelState;
    state.model.tables  = state.model.tables.filter(t => t.name !== tableName);
    state.addedTables   = state.addedTables.filter(n => n !== tableName);
    const dropped = (state.model.columns.find(g => g.table === tableName)?.columns) || [];
    state.model.columns = state.model.columns.filter(g => g.table !== tableName);
    state.addedColumns  = state.addedColumns.filter(c => !dropped.includes(c));
    state.model.joins   = state.model.joins.filter(j => j.leftTable !== tableName && j.rightTable !== tableName);
    delete state.tablePositions[tableName];
    window._pendingManualChanges = true;
    rebuildTablesCanvas();
    rebuildColumnPane();
    rebuildColumnsContent();
  ...
  // Bridge for ColumnTree's checkboxColumns/onToggleColumn (Option 2 only) —
  // also refreshes the canvas so a table card's "added columns" count stays
  // in sync, which the drag-and-drop column flow above never needed to do
  // since it only ever added to the Columns tab, not the canvas cards.
  window._toggleColumnManually = function(tableName, colName, checked) {
    if (checked) addColumnManually(tableName, colName);
    else removeColumnManually(tableName, colName);
    rebuildTablesCanvas();
  };
  ...
  // ── Source schema on load — both variants ──
  // Publishes DATASOURCE_TABLES to the React Tables pane. The 'existing'
  // variant got this for free via flushModelRebuild() below; the blank
  // variant never called a rebuild on load, so its left pane had nothing to
  // list (and so nothing to drag onto the canvas) until the first model edit.
  rebuildColumnPane();
