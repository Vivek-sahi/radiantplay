...
// COPY of DataStudioMVP/components/FilterModal.tsx (2026-09-25, Vivek: "we
// should refer radiant play" — this is the product's `Select value for :`
// dialog, already replicated there). Copied, not imported, per the
// no-cross-prototype-import rule; the one addition is the optional
// `promote` checkbox in the footer (this prototype's promote-to-model
// journey — the MVP has no promote concept by design).
...
export interface FilterModalProps {
  ...
  /**
   * The promote-to-model checkbox (SearchDataOnDataModelFinal's Split preview,
   * promote option only). Omitted — the footer stays exactly the MVP's.
   */
  promote?: {
    checked: boolean;
    onChange: (v: boolean) => void;
    /** The info icon's explanation beside the checkbox. */
    tooltip: string;
  };
  onCancel: () => void;
...
export function FilterModal({ filter, distinctValues, promote, onCancel, onApply }: FilterModalProps) {
    ...
    <Modal
      ...
      footer={(
        <div style={{ display: 'flex', width: '100%', alignItems: 'center', gap: sp.B }}>
          {promote && (
            <div style={{ display: 'flex', alignItems: 'center', gap: sp.B, marginRight: 'auto' }}>
              <Checkbox checked={promote.checked} onChange={promote.onChange} showLabel={false} />
              <span
                onClick={() => promote.onChange(!promote.checked)}
                style={{ fontSize: fs.xs, color: c['content-primary'], fontFamily: ff.primary, cursor: 'pointer' }}
              >
                Add this filter to this model
              </span>
              <Tooltip content={promote.tooltip} placement="top">
                <span style={{ display: 'flex', color: c['content-secondary'], cursor: 'default' }}>
                  <Icon name="info-circle" size="s" color="currentColor" />
                </span>
              </Tooltip>
            </div>
          )}
          <div style={{ display: 'flex', gap: sp.B, marginLeft: 'auto' }}>
