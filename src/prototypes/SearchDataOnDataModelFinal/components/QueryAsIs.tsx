...
import RefreshIcon from './queryAsIsDeps/RefreshIcon';
...
import {
  ...
  IncreaseDecimalIcon, PercentIcon, ReduceDecimalIcon, TextFormatIcon, TextWrapIcon,
} from './icons/SpreadsheetIcons';
import { generateMockRows, isNumericColumn } from './previewMockData';
import styles from './QueryAsIs.module.css';
...
// ─── "Nothing selected" illustration (Split data preview) ────────────────────
// One object, drawn once (2026-09-22, Komal: "don't like the illustration,
// it's too complicated"). The previous pass had two table cards, a join
// connector, a join badge, a focus ring and three result rows — five ideas
// competing inside 112px. This is the single idea the state is actually
// about: an empty data grid, waiting to be filled.
//
// Same vocabulary as the Tables section's own empty-state illustration —
// white card, light grey strokes — and no colour at all, since nothing here
// is an action to take.
const previewEmptyIllustration = (
  <svg className={styles.previewEmptyIllo} width="88" height="60" viewBox="0 0 88 60" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden>
    <rect x="2" y="2" width="84" height="56" rx="5" fill="#FFFFFF" stroke="#DBDFE7" strokeWidth="1.2" />
    {/* Header band */}
    <path d="M2 7A5 5 0 0 1 7 2H81A5 5 0 0 1 86 7V16H2Z" fill="#F6F8FA" />
    <rect x="9" y="7.5" width="14" height="3" rx="1.5" fill="#C0C6CF" />
    <rect x="37" y="7.5" width="14" height="3" rx="1.5" fill="#C0C6CF" />
    <rect x="65" y="7.5" width="12" height="3" rx="1.5" fill="#C0C6CF" />
    {/* The grid itself, left empty — there are no rows to preview yet */}
    <g stroke="#EAEDF2" strokeWidth="1.2">
      <path d="M2 16H86" />
      <path d="M2 30H86" />
      <path d="M2 44H86" />
      <path d="M30 16V58" />
...
const SheetDataPanel: React.FC<{
...
}> = ({ selectedKeys, onToggle, importedCsvGroups = [], savedInputTables = [], activeInputTable = null, style }) => {
  ...
  return (
    ...
    <aside className={styles.dataPanel} style={{
      borderRadius: 0,
      boxShadow: 'none',
      height: '100%',
      ...
      {/* Search + panel toggle */}
      <div className={`${styles.panelSearch} ${styles.sheetPanelSearch}`}>
      ...
      <div className={styles.columnSections}>
        {activeInputTable ? (
          ...
          (() => {
            ...
            return SHEET_PANEL_SECTIONS.filter(s => s.id !== 'custom').map(section => {
              ...
              return (
                <div key={section.id}>
                  <button
                    className={`${styles.sectionHeader} ${styles.sheetPanelHeader}`}
                  ...
                  {panelExpanded.has(section.id) && cols && cols.map(col => (
                    <div key={col.id} className={styles.sheetPanelItem} onClick={() => onToggle(col.id)}>
                      <Checkbox checked={inputColIds.has(col.id)} onChange={() => onToggle(col.id)} showLabel={false} />
        ...
        SHEET_PANEL_SECTIONS.map(section => (
          <div key={section.id}>
            <button
              className={`${styles.sectionHeader} ${styles.sheetPanelHeader}`}
              onClick={() => setPanelExpanded(prev => {
...
const SheetView: React.FC<{
  ...
  style?: React.CSSProperties;
  /**
   * canvasScope only (2026-09-22, Komal: "make the data preview work like an
   * actual data preview"). `columns`/`rows` are otherwise just a seed for
   * this view's OWN column-picker (`selectedColKeys`, restricted to the
   * fixed DATA_MODEL_COLS schema — see `sheetCols` below): a table like
   * "fact_customer" was never in that list, so the filter matched nothing
   * and silently fell back to the hardcoded default retail columns. This
   * bypasses that entirely and renders `columns`/`rows` verbatim.
   */
  canvasScopeMode?: boolean;
  /**
   * Shown in the row area instead of the 60 blank numbered rows when there is
   * nothing to preview (2026-09-22, Komal: "create an empty state for when
   * the user opens the panel and either no tables are added, or no
   * tables/joins are selected"). canvasScope only — leaving it undefined
  ...
  const initColKeys = (): Set<string> => {
    ...
    if (columns.length > 0) {
      const queried = new Set(columns.map(c => c.key).filter(k => DATA_MODEL_COLS.some(dc => dc.key === k)));
  ...
  useEffect(() => {
    if (emptyMode || columns.length === 0) return;
    const queried = new Set(columns.map(c => c.key).filter(k => DATA_MODEL_COLS.some(dc => dc.key === k)));
    if (queried.size > 0) {
  ...
  // Always derive visible sheet columns from DATA_MODEL_COLS + selectedColKeys so the
  // column picker can show / hide any column from the full data model.
  const sheetCols: AnswerColDef[] = canvasScopeMode ? columns : DATA_MODEL_COLS.filter(c => selectedColKeys.has(c.key));

  // Raw (row-level) rows.
  // In emptyMode with no columns selected yet, stay empty.
  // Once the user picks columns via the picker, fall through to SALES_DATA.
  const rawSheetRows: Record<string, unknown>[] = (emptyMode && selectedColKeys.size === 0)
    ? []
    : rows.length > 0
      ? rows
  ...
  const sheetRows = sheetDataView === 'row-level' ? rawSheetRows : aggSheetRows;
  ...
  // There is nothing to preview and a state to show for it instead. The sheet
  // strips itself back to that state alone (2026-09-22, Komal: "fix the
  // pagination and remove the formula row from the empty state"): no formula
  ...
  >
    ...
    <div className={styles.sheetToolbar}>
      ...
      <div className={styles.sheetTbGroup}>
        <TbBtn label="Undo"><Icon name="undo" size="s" /></TbBtn>
        <TbBtn label="Redo"><Icon name="redo" size="s" /></TbBtn>
      </div>
      <div className={styles.sheetTbDivider} />
      {/* Advance sorting, Conditional formatting */}
      <div className={styles.sheetTbGroup}>
        <TbBtn label="Advance sorting"><AdvancedSortingIcon size={14} /></TbBtn>
        <TbBtn label="Conditional formatting"><Icon name="conditional-format" size="s" /></TbBtn>
      </div>
      <div className={styles.sheetTbDivider} />
      {/* Text alignment, Text wrapping */}
      <div className={styles.sheetTbGroup}>
        <TbBtn label="Text alignment"><AlignLeftIcon size={14} /></TbBtn>
        <TbBtn label="Text wrapping"><TextWrapIcon size={14} /></TbBtn>
      </div>
      <div className={styles.sheetTbDivider} />
      {/* Decrease decimal, Increase decimal, Number, Currency, Percent, Format */}
      <div className={styles.sheetTbGroup}>
        <TbBtn label="Decrease decimal"><ReduceDecimalIcon size={14} /></TbBtn>
        <TbBtn label="Increase decimal"><IncreaseDecimalIcon size={14} /></TbBtn>
        ...
        <TbBtn label="Currency"><CurrencyIcon size={14} /></TbBtn>
        <TbBtn label="Percent"><PercentIcon size={14} /></TbBtn>
        <TbBtn label="Format"><TextFormatIcon size={14} /></TbBtn>
      </div>
      <div className={styles.sheetTbDivider} />
      {/* Column sizing, Style, Select columns, Style settings */}
      <div className={styles.sheetTbGroup}>
        <TbBtn label="Column sizing"><ColumnSizeIcon size={14} /></TbBtn>
        ...
        <div style={{ position: 'relative' }} ref={colPickerRef}>
          ...
          {colPickerOpen && (() => {
            const dataModelCols = DATA_MODEL_COLS; // always show the full data model
    ...
    <div className={styles.sheetGrid} style={{ position: 'relative' }}>
      ...
      {/* Column header row */}
      <div className={styles.sheetColHeaderRow} style={showCanvasEmpty ? { display: 'none' } : undefined}>
      ...
      <div className={styles.sheetDataArea}>
        {/* Same treatment the Combined sheet already uses for "Add a table
            first": Radiant NoData laid out side by side, centred in the row
            area, overflow:auto so the copy stays reachable when the panel is
            dragged short. */}
        {emptyMode && allCols.length === 0 && canvasEmptyState
          ? (
    ...
    <div className={styles.sheetPagination}>
      ...
      <div className={styles.sheetPagCenter}>
        ...
        {/* The sheet renders every row it has in one pass, so in canvasScope
...
function aggregate(state: QueryState): AggRow[] {
  let rows = SALES_DATA;
...
function buildDisplayColumns(state: QueryState) {
  ...
  state.metrics.forEach(m => {
    ...
    cols.push({
      key: m, label: METRIC_LABELS[m], sortable: true, align: 'right', width: 160,
...
function buildDisplayData(state: QueryState): Record<string, unknown>[] {
  let rows: Record<string, unknown>[] = SALES_DATA.map((r, i) => ({
    ...r,
    _id: String(i),
  }));
...
function queryToTokens(state: QueryState): SearchToken[] {
  ...
  state.metrics.forEach(m => tokens.push({ id: m, label: METRIC_LABELS[m], type: 'measure' }));
  state.derivedCols.forEach(dc => {
...
function queryToTitle(state: QueryState): string {
  if (!state.metrics.length && !state.derivedCols.length) return 'Answer';
  const metrics = [
    ...state.metrics.map(m => METRIC_LABELS[m]),
...
const AnswerTable: React.FC<{ columns: AnswerColDef[]; data: Record<string, unknown>[] }> = ({ columns, data }) => {
        ...
        <thead>
          <tr>
            {columns.map(col => (
              <th
                key={col.key}
                className={`${styles.answerTh} ${col.align === 'right' ? styles.answerRight : ''}`}
            ...
            <th className={styles.answerThFiller} />
          </tr>
        </thead>
        <tbody>
          {sorted.map((row, ri) => (
...
export interface SearchDataExplorationsProps {
  ...
  /**
   * "Split" layout's Data preview dock only (2026-09-22, Komal: "make the
   * data preview work like an actual data preview. When someone clicks on a
   * table, all the selected columns get displayed... similarly for joins").
   * Read-only here — unlike the sibling SearchDataExplorations.tsx (Combined
   * mode), this file has no scope-switcher UI of its own; the scope only
   * ever changes because the canvas click upstream (PreviewPanel3.tsx)
   * changed it. When provided, the grid shows exactly the selected
   * table's/join's columns (from modelColumns, the same "added to the
   * model" set the canvas cards themselves count) with deterministic mock
   * rows, bypassing the normal search-query-driven SALES_DATA path
  ...
  canvasScope?: {
    ...
    /** 'none' = nothing picked yet (Split's landing state). */
    scope: 'none' | 'table' | 'join' | 'model';
...
export const SearchDataExplorations: React.FC<SearchDataExplorationsProps> = ({ onExit: _onExit, onSave, onSaveChanges: _onSaveChanges, initialSnapshot, mode, showSpotter = true, editMode = false, liveboardName, onOpenInSearchData, onHamburgerClick: _onHamburgerClick, canvasScope }) => {
  ...
  // displaying the data" / "do this for joins as well") — the scope switch is
  // instant in the prototype, and reading as instant made it look like nothing
  // had been fetched. The cover is opaque, so the new columns never flash
  // under it. Keyed on what was selected, so re-clicking the same card or
  // join doesn't re-run it; model scope is unaffected.
  // Nothing to preview — two distinct reasons, so two distinct states
  // (2026-09-22, Komal: "create an empty state for when the user opens the
  // panel and either no tables are added, or no tables/joins are selected").
  // Same Radiant NoData treatment, assets and side-by-side layout the
  ...
  const canvasEmptyState = !hasCanvasScope ? undefined : canvasScope!.tables.length === 0 ? (
    ...
    <NoData
      ...
      description="Select a table or a join to preview its rows."
    />
  );
  const [scopeLoading, setScopeLoading] = useState(false);
  const scopeKey = !hasCanvasScope
    ? null
    : canvasScope!.scope === 'table'
      ? `table:${canvasScope!.selectedTable}`
      : canvasScope!.scope === 'join' && canvasScope!.selectedJoin
        ? `join:${canvasScope!.selectedJoin.leftTable}>${canvasScope!.selectedJoin.rightTable}`
  ...
  // model). Always qualified (table.column), and always limited to columns
  // actually added to the model (canvasScope.modelColumns), same as the
  // canvas cards' own "n/m Columns" count.
  type ScopedCol = { key: string; label: string; table: string; measure: boolean };
  const scopedCols: ScopedCol[] = React.useMemo(() => {
    if (!canvasScope) return [];
    const colsFor = (tableName: string): ScopedCol[] => {
      const inModel = canvasScope.modelColumns.find(g => g.table === tableName)?.columns ?? [];
      return (canvasScope.dataSourceTables.find(d => d.name === tableName)?.columns ?? [])
        .filter(col => inModel.includes(col))
        .map(col => ({
          key: `${tableName}.${col}`,
          label: `${tableName}.${col}`,
          table: tableName,
    ...
    if (canvasScope.scope === 'join' && canvasScope.selectedJoin) {
      return [...colsFor(canvasScope.selectedJoin.leftTable), ...colsFor(canvasScope.selectedJoin.rightTable)];
    }
    // Nothing picked yet — that's what puts the panel into its "Nothing
    // selected" empty state (2026-09-22). Distinct from 'model', which is now
    ...
    if (canvasScope.scope === 'model') {
      ...
      return canvasScope.tables.filter(t => joinedTables.has(t.name)).flatMap(t => colsFor(t.name));
    }
    return colsFor(canvasScope.selectedTable);
  }, [canvasScope]);
  ...
  // ── Worksheet tab toggle (V1 and V3) ─────────────────────────────────────
  const [sheetTab, setSheetTab] = useState<'query' | 'sheet'>(initialSnapshot?.sheetTab ?? 'query');
  ...
  // Data panel for the sheet sub-header (v1 / spreadsheet mode)
  const [sheetDataPanelOpen, setSheetDataPanelOpen] = useState(false);
  // Overflow "more" menus
  ...
  const _allDisplayColumns = hasCanvasScope
    ? scopedCols.map(c => ({
        ...
        label: c.label,
        sortable: true,
        align: c.measure ? 'right' as const : undefined,
            ...
            {hasQuery && (<>
              ...
              {sheetTab !== 'sheet' && <CanvasBlocksPanel blocks={canvasBlocks} />}
                ...
                <div className={styles.tableWrapper}>
                  <AnswerTable columns={displayColumns} data={displayData} />
    ...
    <div className={styles.root}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden', background: '#fff', position: 'relative' }}>
          ...
          {sheetDataPanelOpen && (
            <div style={{ width: 260, flexShrink: 0, borderRight: '1px solid var(--rd-sys-color-border-divider, #eaedf2)', overflowY: 'auto' }}>
              <SheetDataPanel
                selectedKeys={activeCols}
                onToggle={colId => toggleColumn(colId)}
      ...
      <div className={styles.queryBarRow} style={{ position: 'relative', zIndex: (searchFocused || isDirty) ? 110 : 'auto' }}>
        ...
        <div
          ...
          onClick={() => { setSearchFocused(true); setBarActive(true); barInputRef.current?.focus(); }}
        >
          {/* Left: the data-panel toggle alone. The model selector that used
              to sit here is gone (2026-09-22, Komal: "in the Query tab,
              remove the model selector completely... and move the search to
              left") — with it went queryBarLeft's fixed 252px width and the
              divider that closed that column, so the search now starts
        ...
        {/* Undo / redo / reset box */}
        <div className={styles.undoRedoBox}>
          ...
          <button className={styles.iconBtn} aria-label="Reset">
            <RefreshIcon size="m" color={systemColors.light['content-primary']} />
          </button>
        </div>
      ...
      <div className={styles.main}>
        ...
        {<aside className={`${styles.dataPanel}${dataPanelVisible ? '' : ` ${styles.dataPanelHidden}`}`} style={(searchFocused || isDirty) ? { position: 'relative', zIndex: 60 } : undefined}>
                ...
                {section.kind === 'expandable' &&
                  ...
                  section.columns.map(col => (
                    ...
                    >
                      ...
                      <span onClick={e => { e.stopPropagation(); toggleColumn(col.id); }}>
                        <DataToken label={col.label} type={col.type} variant="panel" />
                      </span>
                    </div>
                  ))}
