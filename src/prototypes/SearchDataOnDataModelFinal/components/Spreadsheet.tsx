...
import { fontFamily as ff } from '@tokens/typography';
...
export interface SpreadsheetGridProps {
  ...
  /**
   * MVP only: columns whose header shows a dedicated funnel icon, opening the
   * filter-values popover directly. Hidden on every other column until the
   * column ▾ menu's "Filter" item reveals it (or a filter already exists there).
   */
  visibleFilterCols?: Set<string>;
  /** Columns with a filter added to the model — icon tints blue. */
  addedFilterCols?: Set<string>;
  /** Columns with a draft (not-yet-added) filter — icon tints orange. */
  draftFilterCols?: Set<string>;
  /** (col, x, top, height) of the clicked icon — an anchor rect, not a fixed drop point, so the caller can auto-flip above/below. */
  onFilterIconClick?: (col: string, x: number, top: number, height: number) => void;
}
...
export function SpreadsheetGrid({
  ...
  visibleFilterCols, addedFilterCols, draftFilterCols, onFilterIconClick,
              ...
              return (
                <th key={col} data-col={col} style={{
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', gap: 5, justifyContent: numericTypes.includes(type) ? 'flex-end' : 'flex-start' }}>
                    ...
                    {visibleFilterCols?.has(col) && (() => {
                      // Added-to-model wins visually over draft if a column were ever in
                      // both (it can't be — one filter per column — but keep it defined).
                      const isAdded = addedFilterCols?.has(col);
                      const isDraft = !isAdded && draftFilterCols?.has(col);
                      const restColor = isAdded ? '#2770EF' : isDraft ? '#92640A' : '#A5ACB9';
                      const restBg = isAdded ? '#E7EEFB' : isDraft ? '#FFF3E0' : 'transparent';
                      // A neutral (unfiltered) column previews blue on hover — the same
                      // "available action" affordance as before. A draft column stays
                      // orange on hover; it's already active, just not yet added.
                      const hoverColor = isDraft ? '#92640A' : '#2770EF';
                      const hoverBg = isDraft ? '#FFF3E0' : '#E7EEFB';
                      return (
                        <button
                          onClick={e => { e.stopPropagation(); const r = (e.currentTarget as HTMLElement).getBoundingClientRect(); onFilterIconClick?.(col, r.left, r.top, r.height); }}
                          title={isAdded ? 'Filter (added to model)' : isDraft ? 'Filter (draft)' : 'Filter'}
                          style={{ width: 16, height: 16, flexShrink: 0, border: 'none', background: restBg, borderRadius: 3, color: restColor, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}
                          ...
                          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = restBg; (e.currentTarget as HTMLElement).style.color = restColor; }}
                        >
                          <svg width="10" height="10" viewBox="0 0 16 16" fill="none"><path d="M2 4h12l-4.5 5.5v3.5l-3-1.5v-2L2 4z" stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round"/></svg>
                        </button>
                      );
                    })()}
                    <button
...
export interface DataSheetToolbarProps {
  onDownloadCsv: () => void;
  // Omitted where a full-screen toggle already lives at the Preview panel's own
  // title level (MVP) — showing both there and here duplicated the control.
  onToggleExpand?: () => void;
  expanded: boolean;
  ...
  onFilter?: () => void;
  onFormula?: () => void;
  // Row limit — omitted (as today) unless the caller passes a value; MVP moves its
  // Limit selector down from the Preview panel's title row into this toolbar.
  limit?: number;
  onLimitChange?: (n: number) => void;

  // ── Optimized (SearchDataOnDataModelFinal) only ──────────────────
  // All optional: omitted by DataStudioV2's call site, so its toolbar keeps
  // the original visual-only behaviour. When supplied, these make the
  // formatting chrome real. Formatting targets the selected cell's COLUMN.
  onUndo?: () => void;
  onRedo?: () => void;
  canUndo?: boolean;
  canRedo?: boolean;
  ...
  onWrap?: (w: 'overflow' | 'wrap' | 'clip') => void;
  onCurrency?: () => void;
  onPercent?: () => void;
  onDecimals?: (delta: 1 | -1) => void;
  onNumberFormat?: (fmt: string) => void;
  onFillColor?: (color: string | null) => void;
  /** Column the formatting buttons will act on; undefined = nothing selected. */
  formatTarget?: string;
  /**
   * What kind of column formatTarget is. Drives which formatting controls are
   * live: numbers get currency/percent/decimals and the format menu, dates get
   * the format menu only, text gets neither. Alignment and wrap ignore it —
   * they stay enabled regardless of selection.
   */
  formatTargetType?: 'numeric' | 'date' | 'text';
  /** Trailing slot, pinned to the toolbar's right edge. */
  right?: React.ReactNode;
}

const NUMBER_FORMATS = ['Automatic', 'Number', 'Currency', 'Percent', 'Date'];
// Fill-colour swatches for the Optimized toolbar's Fill button.
const FILL_COLORS = ['#FFFFFF', '#F6F8FA', '#EAF1FE', '#E6F6EC', '#FEF6E0', '#FDECEA'];

export function DataSheetToolbar({
  onDownloadCsv, onToggleExpand, expanded, onFilter, onFormula, limit, onLimitChange,
  onUndo, onRedo, canUndo, canRedo, onSort, onFormatPaint, formatPaintActive,
  onAlign, onWrap, onCurrency, onPercent, onDecimals, onNumberFormat, onFillColor,
  formatTarget, formatTargetType, right,
}: DataSheetToolbarProps) {
  const [formatOpen, setFormatOpen] = React.useState(false);
  const formatBtnRef = React.useRef<HTMLDivElement>(null);
  // Caret dropdowns for Sort / Align / Wrap / Fill. Only rendered when the
  // matching handler is supplied, so DataStudioV2's call site is unaffected.
  const [openMenu, setOpenMenu] = React.useState<null | 'sort' | 'align' | 'wrap' | 'fill'>(null);
  const sortRef = React.useRef<HTMLDivElement>(null);
  const alignRef = React.useRef<HTMLDivElement>(null);
  const wrapRef = React.useRef<HTMLDivElement>(null);
  const fillRef = React.useRef<HTMLDivElement>(null);
  // Formatting acts on the selected cell's column — with nothing selected the
  // format buttons are inert and dimmed rather than silently doing nothing.
  const noTarget = onAlign != null && !formatTarget;
  // Number formatting is type-dependent; alignment and wrap are not.
  const typeAware  = onAlign != null && formatTargetType != null;
  const noNumberFmt = onAlign != null && (!typeAware || formatTargetType !== 'numeric');
  const noFormatMenu = onAlign != null && (!typeAware || (formatTargetType !== 'numeric' && formatTargetType !== 'date'));

  const iconBtn = (title: string, svg: React.ReactNode, onClick?: () => void, opts?: { disabled?: boolean; active?: boolean }) => {
    const disabled = opts?.disabled ?? false;
    return (
      <button onClick={disabled ? undefined : onClick} title={title} aria-disabled={disabled}
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 26, height: 24, padding: 0, borderRadius: 5, border: 'none', background: opts?.active ? '#EAF1FE' : 'transparent', color: disabled ? '#C0C6CF' : opts?.active ? '#2770EF' : '#64748B', cursor: disabled ? 'not-allowed' : 'pointer', fontFamily: ff.primary }}
        onMouseEnter={e => { if (disabled) return; (e.currentTarget as HTMLElement).style.background = opts?.active ? '#EAF1FE' : '#F6F8FA'; (e.currentTarget as HTMLElement).style.color = opts?.active ? '#2770EF' : '#1D232F'; }}
        onMouseLeave={e => { if (disabled) return; (e.currentTarget as HTMLElement).style.background = opts?.active ? '#EAF1FE' : 'transparent'; (e.currentTarget as HTMLElement).style.color = opts?.active ? '#2770EF' : '#64748B'; }}
      >{svg}</button>
    );
  };
  ...
  const iconCaretBtn = (title: string, svg: React.ReactNode, onClick?: () => void, opts?: { disabled?: boolean }) => {
    const disabled = opts?.disabled ?? false;
    return (
      <button onClick={disabled ? undefined : onClick} title={title} aria-disabled={disabled}
        style={{ display: 'flex', alignItems: 'center', gap: 1, height: 24, padding: '0 3px 0 6px', borderRadius: 5, border: 'none', background: 'transparent', color: disabled ? '#C0C6CF' : '#64748B', cursor: disabled ? 'not-allowed' : 'pointer', fontFamily: ff.primary }}
  ...
  const menuItem = (label: string, onPick: () => void) => (
    <button key={label} onClick={() => { onPick(); setOpenMenu(null); }}
      style={{ display: 'flex', alignItems: 'center', width: '100%', padding: '6px 12px', border: 'none', background: 'transparent', cursor: 'pointer', textAlign: 'left', fontFamily: ff.primary, fontSize: 12, fontWeight: 500, color: '#1D232F' }}
      onMouseEnter={e => (e.currentTarget.style.background = '#F6F8FA')}
      onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
    >{label}</button>
  );
  const MENU_STYLE: React.CSSProperties = { background: '#fff', border: '1px solid #E2E6EC', borderRadius: 8, boxShadow: '0 6px 24px rgba(25,35,49,0.13)', width: 150, padding: '4px 0' };
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 1, padding: '4px 10px', borderBottom: BORDER, background: '#fff', flexShrink: 0, overflowX: 'auto' }}>
      {iconBtn('Undo', <svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M9 14 4 9l5-5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/><path d="M4 9h10.5a5.5 5.5 0 0 1 0 11H11" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/></svg>, onUndo, { disabled: onUndo != null && !canUndo })}
      {iconBtn('Redo', <svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M15 14l5-5-5-5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/><path d="M20 9H9.5a5.5 5.5 0 0 0 0 11H13" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/></svg>, onRedo, { disabled: onRedo != null && !canRedo })}
      {divider}
      <div ref={sortRef} style={{ position: 'relative' }}>
        {iconCaretBtn('Sort range', <AdvancedSortingIcon size={14} />, onSort ? () => setOpenMenu(m => m === 'sort' ? null : 'sort') : undefined, { disabled: noTarget })}
        {openMenu === 'sort' && onSort && (
          <AnchoredMenu open anchorRef={sortRef} onClose={() => setOpenMenu(null)} placement="bottom-start" style={MENU_STYLE}>
            {menuItem('Sort A \u2192 Z', () => onSort('asc'))}
            {menuItem('Sort Z \u2192 A', () => onSort('desc'))}
            {menuItem('Clear sort', () => onSort('clear'))}
          </AnchoredMenu>
        )}
      </div>
      {divider}
      {iconBtn(formatPaintActive ? 'Format paint — pick a column to paint onto' : 'Format paint', <StyleIcon size={14} />, onFormatPaint, { disabled: noTarget && !formatPaintActive, active: formatPaintActive })}
      {divider}
      <div ref={alignRef} style={{ position: 'relative' }}>
        {iconCaretBtn('Align', <AlignLeftIcon size={14} />, onAlign ? () => setOpenMenu(m => m === 'align' ? null : 'align') : undefined)}
        {openMenu === 'align' && onAlign && (
          <AnchoredMenu open anchorRef={alignRef} onClose={() => setOpenMenu(null)} placement="bottom-start" style={MENU_STYLE}>
            {menuItem('Left', () => onAlign('left'))}
            {menuItem('Center', () => onAlign('center'))}
            {menuItem('Right', () => onAlign('right'))}
          </AnchoredMenu>
        )}
      </div>
      <div ref={wrapRef} style={{ position: 'relative' }}>
        {iconCaretBtn('Wrap text', <TextWrapIcon size={14} />, onWrap ? () => setOpenMenu(m => m === 'wrap' ? null : 'wrap') : undefined)}
        {openMenu === 'wrap' && onWrap && (
          <AnchoredMenu open anchorRef={wrapRef} onClose={() => setOpenMenu(null)} placement="bottom-start" style={MENU_STYLE}>
            {menuItem('Overflow', () => onWrap('overflow'))}
            {menuItem('Wrap', () => onWrap('wrap'))}
            {menuItem('Clip', () => onWrap('clip'))}
          </AnchoredMenu>
        )}
      </div>
      {divider}
      {iconBtn('Format as currency', <CurrencyIcon size={14} />, onCurrency, { disabled: noNumberFmt })}
      {iconBtn('Format as percent', <PercentIcon size={14} />, onPercent, { disabled: noNumberFmt })}
      {iconBtn('Decrease decimal places', <ReduceDecimalIcon size={14} />, () => onDecimals?.(-1), { disabled: noNumberFmt })}
      {iconBtn('Increase decimal places', <IncreaseDecimalIcon size={14} />, () => onDecimals?.(1), { disabled: noNumberFmt })}
      <div ref={formatBtnRef} style={{ position: 'relative' }}>
        <button onClick={() => { if (noFormatMenu) return; setFormatOpen(o => !o); }} title="Number format"
          aria-disabled={noFormatMenu}
          style={{ display: 'flex', alignItems: 'center', gap: 2, height: 24, padding: '0 6px', borderRadius: 5, border: 'none', background: formatOpen ? '#F6F8FA' : 'transparent', color: noFormatMenu ? '#C0C6CF' : (formatOpen ? '#1D232F' : '#64748B'), fontSize: 11, fontWeight: 600, cursor: noFormatMenu ? 'not-allowed' : 'pointer', fontFamily: ff.primary }}
          onMouseEnter={e => { if (!noFormatMenu) (e.currentTarget as HTMLElement).style.background = '#F6F8FA'; }}
        ...
        {formatOpen && (
          <AnchoredMenu open={formatOpen} anchorRef={formatBtnRef} onClose={() => setFormatOpen(false)} placement="bottom-start" style={{ background: '#fff', border: '1px solid #E2E6EC', borderRadius: 8, boxShadow: '0 6px 24px rgba(25,35,49,0.13)', width: 150, padding: '4px 0' }}>
            {NUMBER_FORMATS.map(f => (
              <button key={f} onClick={() => { onNumberFormat?.(f); setFormatOpen(false); }}
                style={{ display: 'flex', alignItems: 'center', width: '100%', padding: '6px 12px', border: 'none', background: 'transparent', cursor: 'pointer', textAlign: 'left', fontFamily: ff.primary, fontSize: 12, fontWeight: 500, color: '#1D232F' }}
                onMouseEnter={e => (e.currentTarget.style.background = '#F6F8FA')}
                onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
              >{f}</button>
      ...
      <div ref={fillRef} style={{ position: 'relative' }}>
        {iconBtn('Fill color', <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M4 8.5 8.5 4l4.5 4.5-4.5 4.5L4 8.5z" stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round"/><path d="M3 11c-.6.6-1 1.2-1 1.8C2 13.7 2.9 14 3.5 14s1.5-.3 1.5-1.2c0-.6-.4-1.2-1-1.8" fill="currentColor"/></svg>, onFillColor ? () => setOpenMenu(m => m === 'fill' ? null : 'fill') : undefined, { disabled: noTarget })}
        {openMenu === 'fill' && onFillColor && (
          <AnchoredMenu open anchorRef={fillRef} onClose={() => setOpenMenu(null)} placement="bottom-start" style={{ ...MENU_STYLE, width: 'auto', padding: 8 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 20px)', gap: 6 }}>
              {FILL_COLORS.map(c => (
                <button key={c} title={c} onClick={() => { onFillColor(c); setOpenMenu(null); }}
                  style={{ width: 20, height: 20, borderRadius: 4, border: '1px solid #E2E6EC', background: c, cursor: 'pointer', padding: 0 }} />
              ))}
            </div>
            <button onClick={() => { onFillColor(null); setOpenMenu(null); }}
              style={{ display: 'flex', alignItems: 'center', width: '100%', marginTop: 8, padding: '5px 4px', border: 'none', background: 'transparent', cursor: 'pointer', textAlign: 'left', fontFamily: ff.primary, fontSize: 12, fontWeight: 500, color: '#1D232F' }}
              onMouseEnter={e => (e.currentTarget.style.background = '#F6F8FA')}
              onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
            >Reset</button>
          </AnchoredMenu>
      ...
      {onToggleExpand && iconBtn(expanded ? 'Exit full width' : 'Full width', expanded ? (
...
export interface SpreadsheetColumnMenuProps {
  ...
  extended?: boolean;
  // MVP only: adds the Clean flyout (Fix nulls/Change type/Replace value/Text case/
  // Trim) into the extended menu — that's where the card's own Clean submenu moved
  // to. The non-extended menu (below) already has its own Clean entry regardless.
  showClean?: boolean;
  /** Default true. MVP sets this false — Filter moves to its own always-visible
   * inline toolbar (opened from a dedicated header icon) instead of living here. */
  showFilter?: boolean;
}

export function SpreadsheetColumnMenu({ menu, onClose, sort, onSort, onHide, onAddFormula, onFilter, onClean, cleanOptions, cleanSubOpen, setCleanSubOpen, extended, showClean, showFilter = true }: SpreadsheetColumnMenuProps) {
      ...
      <div ref={menuRef} style={{ position: 'fixed', left: clampedPos?.left ?? menu.x, top: clampedPos?.top ?? menu.y, visibility: clampedPos ? 'visible' : 'hidden', zIndex: 401, background: '#fff', border: '1px solid #E2E6EC', borderRadius: 10, boxShadow: '0 8px 28px rgba(25,35,49,0.16), 0 1px 4px rgba(25,35,49,0.06)', width: extended ? 240 : 214, padding: '4px 0', fontFamily: ff.primary }}>
        {extended ? (
          <>
            ...
            {divider}
            {showFilter && item(<svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M2 4h12l-4.5 5.5v3.5l-3-1.5v-2L2 4z" stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round"/></svg>, 'Filter', onFilter)}
            {/* Sort — hover submenu */}
            ...
            {/* Clean — hover submenu, same shape as Sort/Text wrapping above. MVP moves
                this down from the card's own menu into the column it actually acts on. */}
            {showClean && (
...
// ── Filter values popover (MVP) ────────────────────────────────────────────────
// Opened from a column header's dedicated funnel icon instead of the column ▾
// menu. A small anchored panel, not a modal — no backdrop, doesn't block the
// rest of the sheet, closes on Cancel/outside click without committing. Include/
// Exclude + a searchable checkbox list of the column's distinct values, matching
// the reference "product" filter (Include/Exclude tabs, search, N-selected +
// select-all, scrollable list, Cancel/Apply) rather than a single operator+value.
// Positioned via AnchoredMenu so it auto-flips above the icon when there isn't
// room below — the small docked preview panel has little room under the header
// row, so opening downward would cover the very rows being filtered.
export type FilterMenuAnchor = { col: string; x: number; y: number; height: number } | null;

export interface FilterValuesMenuProps {
  menu: FilterMenuAnchor;
  columnLabel: string;
  values: string[];
  selected: Set<string>;
  ...
  /** Unchecked (default) = draft: the filter is configured but stays out of the
...
export function FilterValuesMenu({
  ...
  const allShownSelected = shown.length > 0 && shown.every(v => selected.has(v));
  const tabStyle = (active: boolean): React.CSSProperties => ({
    padding: '6px 2px', border: 'none', background: 'transparent', cursor: 'pointer',
    fontFamily: ff.primary, fontSize: 13, fontWeight: 500,
    color: active ? '#2770EF' : '#1D232F',
    borderBottom: active ? '2px solid #2770EF' : '2px solid transparent',
  });
  return (
    <AnchoredMenu
      open={!!menu}
      anchorPoint={menu ? { x: menu.x, y: menu.y, height: menu.height } : null}
      onClose={onCancel}
      placement="bottom-start"
      style={{
        width: 280,
        background: '#fff', border: '1px solid #E2E6EC', borderRadius: 10,
        boxShadow: '0 8px 28px rgba(25,35,49,0.16), 0 1px 4px rgba(25,35,49,0.06)',
        display: 'flex', flexDirection: 'column', maxHeight: 360, fontFamily: ff.primary,
      }}
    >
        <div style={{ padding: '10px 14px 0', fontSize: 14, fontWeight: 700, color: '#1D232F', flexShrink: 0 }}>{columnLabel}</div>
        <div style={{ display: 'flex', gap: 16, padding: '6px 14px 0', borderBottom: BORDER, flexShrink: 0 }}>
          <button style={tabStyle(mode === 'include')} onClick={() => onModeChange('include')}>Include</button>
          <button style={tabStyle(mode === 'exclude')} onClick={() => onModeChange('exclude')}>Exclude</button>
        </div>
        <div style={{ padding: '10px 14px 6px', flexShrink: 0 }}>
          <div style={{ position: 'relative' }}>
            <span style={{ position: 'absolute', left: 9, top: '50%', transform: 'translateY(-50%)', color: '#A5ACB9', display: 'flex' }}>
              <svg width="12" height="12" viewBox="0 0 16 16" fill="none"><circle cx="7" cy="7" r="4.5" stroke="currentColor" strokeWidth="1.3"/><path d="M13 13l-2.5-2.5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg>
            </span>
            <input
              value={search}
              onChange={e => onSearchChange(e.target.value)}
              placeholder="Search"
              style={{ width: '100%', boxSizing: 'border-box', border: BORDER, borderRadius: 6, padding: '6px 10px 6px 28px', fontSize: 12.5, color: '#1D232F', fontFamily: ff.primary, outline: 'none' }}
            />
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 14px', borderTop: BORDER, borderBottom: BORDER, flexShrink: 0 }}>
          <input
            type="checkbox"
            checked={allShownSelected}
            onChange={e => onToggleAll(e.target.checked)}
            style={{ width: 14, height: 14, cursor: 'pointer', accentColor: '#2770EF', flexShrink: 0 }}
          />
          <span style={{ fontSize: 12.5, fontWeight: 700, color: '#1D232F' }}>{selected.size} selected</span>
        </div>
        <div style={{ flex: 1, overflow: 'auto', minHeight: 40 }}>
          {shown.length === 0 ? (
            <div style={{ padding: '14px', textAlign: 'center', fontSize: 12, color: '#A5ACB9' }}>No matching values</div>
          ) : shown.map(v => (
            <label key={v} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 14px', cursor: 'pointer' }}
              onMouseEnter={e => (e.currentTarget.style.background = '#F6F8FA')}
              onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
            >
              <input
                ...
                style={{ width: 14, height: 14, cursor: 'pointer', accentColor: '#2770EF', flexShrink: 0 }}
              />
              <span style={{ fontSize: 12.5, color: '#1D232F', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{v}</span>
            </label>
          ))}
        </div>
        {!hideAddToModel && (
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px 0', borderTop: BORDER, cursor: 'pointer', flexShrink: 0 }}>
          <input
            type="checkbox"
            checked={addToModel}
            onChange={e => onAddToModelChange(e.target.checked)}
            style={{ width: 14, height: 14, cursor: 'pointer', accentColor: '#2770EF', flexShrink: 0 }}
          />
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, padding: '10px 14px', flexShrink: 0 }}>
          <button
            onClick={onCancel}
            style={{ padding: '6px 14px', borderRadius: 7, border: BORDER, background: '#fff', color: '#1D232F', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', fontFamily: ff.primary }}
          >Cancel</button>
          <button
            onClick={onApply}
            style={{ padding: '6px 14px', borderRadius: 7, border: 'none', background: '#2770EF', color: '#fff', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', fontFamily: ff.primary }}
          >Apply</button>
        </div>
    </AnchoredMenu>
  );
