...
import { generateMockRows, isNumericColumn, isDateColumn } from './components/previewMockData';
import {
  Button,
  Tabs,
  Checkbox,
...
const SheetView: React.FC<{
  ...
  // canvasScope (Optimized) only: nothing has been dragged onto the model
  // canvas yet, so there is no data to preview. Swaps the sheet's row area
  // for the Radiant NoData empty state; the toolbar, column header and
  // pagination footer are left untouched. Defaults to false everywhere else.
  noTablesOnCanvas?: boolean;
  ...
  /** Trailing slot for the real toolbar — carries the answer's ⋯ menu. */
  ...
  // Column picked by clicking its header. Exclusive with selectedCell — one or
  // the other, never both — so inline cell editing can't run while a whole
  // column is highlighted. Drives which toolbar controls are enabled.
  ...
  // ── Toolbar enablement from the selected column ──────────────────────────
  // Always on: undo, redo, alignment, wrap, filter columns, fx, export.
  // On once any column is selected: sort, conditional formatting, filter.
  // By type: $ % and the two decimal controls are numeric-only; the 123 format
  // control also covers dates; text columns enable none of them.
  ...
  // Clicking a column header targets that column; otherwise fall back to the
  // selected cell's column, which is how formatting was targeted before.
  const fmtTargetKey: string | undefined = selectedColKey ?? (selectedCell ? allCols[selectedCell.c]?.key : undefined);
  // Type of the targeted column, for the toolbar's number-format controls.
  // Tested against the BARE column name: labels here are qualified
  // ("fact_sales.channel"), and isNumericColumn matches "sales" in the table
  // name, which classified every column of fact_sales as a number.
  const fmtTargetColType: 'numeric' | 'date' | 'text' | undefined = (() => {
    if (!fmtTargetKey) return undefined;
    if (fmtTargetKey.startsWith('__formula_')) return 'numeric';
    const label = allCols.find(c => c.key === fmtTargetKey)?.label ?? '';
    const bare = label.slice(label.lastIndexOf('.') + 1);
    if (isDateColumn(bare)) return 'date';
    if (isNumericColumn(bare)) return 'numeric';
    return 'text';
  })();
  ...
  >
    ...
    {canvasScopeMode ? (
      <DataSheetToolbar
        ...
        formatTargetType={fmtTargetColType}
        onSort={dir => {
          if (!fmtTargetKey) return;
    ...
    {(() => {
      ...
      void (inlineFxColName ?? (isCreating ? activeFormula!.name : selectedColLabel));
      // Nothing in the bar yet — the state both reference images show.
      const fxEmpty = !editingFormulaCell && !fxInput && !activeFormula?.tokens.length;
      return (
        <div className={`${styles.sheetFormulaBar} ${fxFocus ? styles.sheetFormulaBarFocus : ''} ${activeFormulaHasError ? styles.sheetFormulaBarError : ''} ${!canAddFormula ? styles.sheetFormulaBarDisabled : ''}`}>
          {/* Starts at fx — the old 200px "Formula name" section was removed. */}
          {/* Section 1: fx icon — static, no interactivity */}
          ...
          >
            <span className={styles.fbEquals} aria-hidden>=</span>
            ...
            <input
              ...
              placeholder={fxEmpty ? 'Enter formula or value' : ''}
              readOnly={!isCreating}
              spellCheck={false}
              /* width:0 when there's already formula content, so the input doesn't
                 add a gap after the tokens; full width while empty so the
                 placeholder is visible, which is the resting/disabled state. */
              style={{ minWidth: 4, flex: isCreating || fxEmpty ? 1 : 'unset', width: isCreating || fxEmpty ? undefined : 0 }}
            />
    ...
    <div className={styles.sheetGrid} style={{ position: 'relative' }}>
      ...
      <div className={styles.sheetColHeaderRow}>
        ...
        {allCols.map(col => {
          ...
          return (
            <div
              key={col.key}
              className={`${styles.sheetColHeader} ${isFormula ? styles.sheetColHeaderFormula : ''} ${isFormula && formulaErrorKeys.has(col.key) ? styles.sheetColHeaderError : ''} ${selectedColKey === col.key ? styles.sheetColHeaderSelected : ''}`}
              style={{ width: col.width ?? 140, minWidth: col.width ?? 140, ...headerAccentStyle }}
              // Selecting a column clears any selected cell. The chevron and the
              // filter indicator stop propagation, so they keep their own jobs.
              onClick={() => {
                setSelectedColKey(k => (k === col.key ? null : col.key));
      ...
      <div className={styles.sheetDataArea}>
        {/* Radiant NoData, with the DME's own empty-state table icon in the
            illustration slot at its native 32px — the same asset and size the
            Columns and Formulas tabs use. overflow:auto so the copy is still
            reachable if the panel is dragged very short. */}
        {noTablesOnCanvas ? (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', overflow: 'auto' }}>
            <NoData
              ...
              description="Drag a table from the left pane onto the canvas to preview your data."
            />
          </div>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    ...hoverIndicatorStyle,
                    ...cursorStyle,
                    // Whole-column highlight when its header is selected.
                    ...(selectedColKey === col.key ? { background: 'var(--rd-sys-color-background-brand-subtle, #eef3fd)' } : {}),
                  }}
...
export interface SearchDataExplorationsProps {
  ...
  canvasScope?: {
    ...
    // column tree. The preview lists only these, not the whole source schema.
    modelColumns: { table: string; columns: string[] }[];
    /** Saved formulas — listed as their own category beside Measures/Attributes. */
...
export const SearchDataExplorations: React.FC<SearchDataExplorationsProps> = ({ onExit, onSave, onSaveChanges, initialSnapshot, mode, showSpotter = true, editMode = false, liveboardName, onOpenInSearchData, sheetTab: sheetTabProp, onSheetTabChange, hideSheetToggle = false, hideHeaderBar = false, edgeToEdge = false, hideColumnPanelTabs = false, compactPanelSearch = false, alignColumnCheckboxes = false, lightAnswerTableHeader = false, canvasScope }) => {
  ...
  // down, next to the rest of the sheet-view state); left here now that
  // scopedCols no longer branches on it, since the rest of the file reads it.
  ...
  const scopedCols: ScopedCol[] = useMemo(() => {
    ...
    const colsFor = (tableName: string, qualify: boolean): ScopedCol[] => {
      // Only columns added to the model (Komal: "It should only show columns
      // that are in the model... the ones I select here and add"), in the
      // source schema's own order rather than tick order.
    ...
    // Both tabs follow the canvas scope. Spreadsheet used to bypass it and
    // list every table's columns; Komal asked for it to scope too, with Model
    // scope (click empty canvas) staying the all-columns view, so every column
    // is still reachable — just via the canvas selection.
    ...
    if (canvasScope.scope === 'model') {
      // A model's columns come from joined tables. One table on its own is a
      // valid model, so it contributes its columns; past that, only tables that
      // actually take part in a join do — an unjoined table has no defined
      // relationship to the rest, so its columns aren't model columns yet.
      if (canvasScope.tables.length <= 1) {
        return canvasScope.tables.flatMap(t => colsFor(t.name, true));
      }
      const joinedTables = new Set<string>();
      canvasScope.joins.forEach(j => { joinedTables.add(j.leftTable); joinedTables.add(j.rightTable); });
      return canvasScope.tables.filter(t => joinedTables.has(t.name)).flatMap(t => colsFor(t.name, true));
    }
    // Qualified (fact_sales.amount) in every scope, not just join/model —
    // Komal: "full names everywhere".
    return colsFor(canvasScope.selectedTable, true);
  }, [canvasScope]);

  // A formula applies to the whole model, so it needs a model that actually
  // holds together: every table joined in. One table on its own is a model in
  // its own right, so it qualifies. Stricter than the Model-scope column rule
  // above, which still lists the joined tables' columns while a stray table
  // waits to be joined — here a single unjoined table blocks formulas outright.
  const modelFullyJoined = useMemo(() => {
    if (!canvasScope) return true;
    if (canvasScope.tables.length <= 1) return true;
    const joined = new Set<string>();
    canvasScope.joins.forEach(j => { joined.add(j.leftTable); joined.add(j.rightTable); });
    return canvasScope.tables.every(t => joined.has(t.name));
  }, [canvasScope]);

  // Identity of the current canvas selection — drives the reset below.
  const canvasScopeKey = useMemo(() => {
    if (!canvasScope) return '';
    if (canvasScope.scope === 'join' && canvasScope.selectedJoin) {
      return `join:${canvasScope.selectedJoin.leftTable}::${canvasScope.selectedJoin.rightTable}`;
    }
    if (canvasScope.scope === 'model') return 'model';
    return `table:${canvasScope.selectedTable}`;
  }, [canvasScope]);
  ...
  // Formulas get their own category alongside Measures and Attributes, on both
  // the Query and Spreadsheet tabs.
  const formulaCategoryCols = useMemo(
    () => (canvasScope?.modelFormulas ?? []).map(f => ({ id: `__mf_${f.name}`, label: f.name, type: 'measure' as TokenType })),
    [canvasScope?.modelFormulas]
  );
  ...
  const dynamicSections: SectionDef[] = useMemo(() => [
    ...
    ...(formulaCategoryCols.length ? [{ id: 'formulas', label: 'Formulas', kind: 'expandable' as const, columns: formulaCategoryCols }] : []),
  ], [scopedCols, formulaCategoryCols]);
  ...
  const dynamicSheetPanelSections = useMemo(() => [
    ...
    ...(formulaCategoryCols.length ? [{ id: 'formulas', label: 'Formulas', items: formulaCategoryCols }] : []),
  ], [scopedCols, formulaCategoryCols]);
  ...
  // Changing the canvas selection empties the sheet, so what's ticked always
  // matches what's in scope (Komal: "clear them on scope change"). Clearing
  // sheetColKeys also flows into pendingQuery through the effect further down.
  useEffect(() => {
    if (!hasCanvasScope) return;
    setSheetColKeys(new Set());
  }, [canvasScopeKey, hasCanvasScope]);
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden', background: '#fff', position: 'relative' }}>
          ...
          <div style={{ flex: 1, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <SheetView
              ...
              noTablesOnCanvas={hasCanvasScope && canvasScope.tables.length === 0}
              ...
              canAddFormula={!canvasScope || (canvasScope.scope === 'model' && modelFullyJoined)}
              canvasScopeMode={hasCanvasScope}
              toolbarRight={hasCanvasScope ? (
