...
export interface DataSheetToolbarProps {
  ...
  // ── Optimized (SearchDataOnDataModelexplorations) only ──────────────────
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 1, padding: '4px 10px', borderBottom: BORDER, background: '#fff', flexShrink: 0, overflowX: 'auto' }}>
      ...
      <div ref={alignRef} style={{ position: 'relative' }}>
        {iconCaretBtn('Align', <AlignLeftIcon size={14} />, onAlign ? () => setOpenMenu(m => m === 'align' ? null : 'align') : undefined, { disabled: noTarget })}
      ...
      <div ref={wrapRef} style={{ position: 'relative' }}>
        {iconCaretBtn('Wrap text', <TextWrapIcon size={14} />, onWrap ? () => setOpenMenu(m => m === 'wrap' ? null : 'wrap') : undefined, { disabled: noTarget })}
      ...
      {divider}
      {iconBtn('Format as currency', <CurrencyIcon size={14} />, onCurrency, { disabled: noTarget })}
      {iconBtn('Format as percent', <PercentIcon size={14} />, onPercent, { disabled: noTarget })}
      {iconBtn('Decrease decimal places', <ReduceDecimalIcon size={14} />, () => onDecimals?.(-1), { disabled: noTarget })}
      {iconBtn('Increase decimal places', <IncreaseDecimalIcon size={14} />, () => onDecimals?.(1), { disabled: noTarget })}
