...
const EvalView: React.FC<{ onEvaluated?: () => void }> = ({ onEvaluated }) => {
  ...
  if (phase === 'idle') {
    return (
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '56px 24px', fontFamily: FONT, background: c['background-subtle'] }}>
        <div style={{ width: '100%', maxWidth: 520, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 17, fontWeight: 700, color: c['content-primary'], letterSpacing: '-0.3px' }}>Evaluate the model</div>
          <div style={{ fontSize: 12.5, color: '#64748B', lineHeight: 1.55, marginTop: sp.B, maxWidth: 440, marginLeft: 'auto', marginRight: 'auto' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: sp.B, marginTop: sp.D, marginBottom: sp.E }}>
            {SECTIONS.map(s => (
              <div key={s.key} style={{ flex: 1, maxWidth: 220, border: BORDER, borderRadius: 10, background: '#fff', padding: '12px 14px', textAlign: 'left' }}>
                <div style={{ fontSize: 12.5, fontWeight: 600, color: c['content-primary'] }}>{s.label}</div>
                <div style={{ fontSize: 12, color: '#8B96A5', lineHeight: 1.5, marginTop: sp.A }}>{s.desc}</div>
              </div>
            ))}
          </div>
          <button
            onClick={runScan}
            style={{ height: 38, padding: '0 20px', borderRadius: 8, border: 'none', background: '#2770EF', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: FONT, display: 'inline-flex', alignItems: 'center', gap: sp.B }}
    ...
    return (
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '56px 24px', fontFamily: FONT, background: c['background-subtle'] }}>
        ...
        <div style={{ width: '100%', maxWidth: 460, background: '#fff', border: BORDER, borderRadius: 12, boxShadow: '0 1px 4px rgba(25,35,49,0.05)', padding: '18px 20px' }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: c['content-primary'], marginBottom: sp.D }}>Evaluating model…</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: sp.C }}>
            {SCAN_STEPS.map((label, i) => {
              const done = i < scanStep;
              const active = i === scanStep;
              return (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: sp.C, opacity: i <= scanStep ? 1 : 0.4 }}>
                  <span style={{ width: 15, height: 15, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {done ? (
                      ...
                      <span style={{ width: 12, height: 12, borderRadius: '50%', border: `1.6px solid ${c['border-subtle-hover']}`, borderTopColor: '#2770EF', animation: 'ev-spin 700ms linear infinite' }} />
                    ) : (
                      <span style={{ width: 6, height: 6, borderRadius: '50%', background: c['background-subtle'] }} />
                    )}
                  </span>
                  <span style={{ fontSize: 12.5, color: done ? '#8B96A5' : c['content-primary'] }}>{label}</span>
                </div>
              );
            })}
          </div>
  ...
  // ── Results ───────────────────────────────────────────────────────────────
  return (
    <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '20px 24px 28px', fontFamily: FONT, background: c['background-subtle'] }}>
      <style>{`@keyframes ev-spin{to{transform:rotate(360deg)}}`}</style>
      <div style={{ maxWidth: 720, margin: '0 auto' }}>
        {/* Health summary */}
        <div style={{ display: 'flex', alignItems: 'center', gap: sp.C, padding: '14px 16px', border: BORDER, borderRadius: 12, background: allFixed ? 'rgba(6,191,127,0.06)' : '#fff', boxShadow: '0 1px 4px rgba(25,35,49,0.05)', marginBottom: sp.D, transition: 'background 300ms' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: c['content-primary'], letterSpacing: '-0.2px' }}>
              {allFixed ? 'Structure checked · model is sound' : `${totalFindings - fixedCount} issue${totalFindings - fixedCount === 1 ? '' : 's'} found`}
            </div>
            <div style={{ fontSize: 12, color: '#64748B', marginTop: sp.A }}>
        ...
        {SECTIONS.map(sec => {
          ...
          return (
            <div key={sec.key} style={{ border: BORDER, borderRadius: 12, background: '#fff', overflow: 'hidden', marginBottom: sp.D, boxShadow: '0 1px 4px rgba(25,35,49,0.05)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: sp.C, padding: '11px 15px', borderBottom: BORDER }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: c['content-primary'] }}>{sec.label}</div>
                  <div style={{ fontSize: 12, color: '#8B96A5', marginTop: 1 }}>{sec.desc}</div>
                </div>
                {pending > 0 ? (
                  <button
                  ...
                  <span style={{ fontSize: 12, fontWeight: 600, color: '#06BF7F', display: 'flex', alignItems: 'center', gap: sp.A, flexShrink: 0 }}>
              ...
              {sec.findings.map(f => {
                ...
                return (
                  <div key={f.id} style={{ padding: '11px 15px', borderBottom: `1px solid ${c['border-divider']}`, display: 'flex', gap: sp.C, alignItems: 'flex-start' }}>
                    <div style={{ width: 18, height: 18, borderRadius: '50%', flexShrink: 0, marginTop: 1, border: st === 'done' ? 'none' : `1.5px solid ${st === 'fixing' ? '#2770EF' : c['border-subtle-hover']}`, background: st === 'done' ? '#06BF7F' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: st === 'fixing' ? 'ev-spin 700ms linear infinite' : 'none' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: sp.B, marginBottom: sp.A }}>
                        <span style={{ fontSize: 12.5, fontWeight: 600, color: c['content-primary'] }}>{f.title}</span>
                        {st !== 'done' && <span style={{ fontSize: 12, fontWeight: 700, padding: '1px 5px', borderRadius: 4, color: sev.fg, background: sev.bg, textTransform: 'uppercase', letterSpacing: '0.05em', flexShrink: 0 }}>{sev.label}</span>}
                      </div>
                      <div style={{ fontSize: 12, color: '#8B96A5', lineHeight: 1.5 }}>{st === 'done' ? f.fix : f.detail}</div>
                    </div>
                    {st === 'pending' && (
                      <button
                        onClick={() => fixOne(f.id)}
                        style={{ flexShrink: 0, height: 26, padding: '0 11px', borderRadius: 6, border: `1px solid ${c['border-subtle-hover']}`, background: '#fff', color: '#2770EF', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: FONT }}
                    ...
                    {st === 'fixing' && <span style={{ flexShrink: 0, fontSize: 12, fontWeight: 600, color: '#2770EF', paddingTop: sp.A }}>Fixing…</span>}
                  </div>
                );
              })}
            </div>
