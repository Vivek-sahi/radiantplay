...
const ConfidenceBadge: React.FC<{ value: number; reasoning?: string }> = ({ value, reasoning }) => {
  ...
  return (
    ...
    >
      <span style={{ fontSize: 12, fontWeight: fw.semibold, color, backgroundColor: bg, padding: '2px 6px', borderRadius: 4, whiteSpace: 'nowrap' as const, cursor: 'default' }}>
        {value}%
      </span>
      {reasoning && tip && (
        <div style={{
          position: 'absolute', bottom: '100%', right: 0, marginBottom: sp.B,
          backgroundColor: c['background-base-inverse'], color: c['content-secondary'],
...
const SectionHeader: React.FC<{ title: string; subtitle: string }> = ({ title, subtitle }) => (
  <div style={{ paddingTop: sp.D, paddingBottom: sp.C }}>
    ...
    <div style={{ fontSize: fs.xs, color: c['content-secondary'], marginTop: sp.A, lineHeight: '16px' }}>{subtitle}</div>
  </div>
);
...
const Callout: React.FC<{ title: string; body: string }> = ({ title, body }) => (
  <div style={{
    ...
    <div style={{ fontSize: fs.xs, color: '#92400E', lineHeight: '18px', marginTop: sp.A }}>{body}</div>
  </div>
);
...
const DataTable: React.FC<{
  ...
  <div style={{ border: `1px solid ${c['border-divider']}`, borderRadius: 8, overflow: 'hidden' }}>
    <div style={{
      ...
      {headers.map((h, i) => (
        <span key={i} style={{ fontSize: 12, fontWeight: fw.medium, color: c['content-secondary'] }}>{h}</span>
      ))}
    </div>
    {rows.map((row, ri) => (
      <div key={ri} style={{
  ...
  return (
    <div style={{ marginBottom: sp.D }}>
      ...
      >
        ...
        {t.connection && (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: sp.A, fontSize: 12, fontWeight: fw.medium, color: c['content-secondary'], backgroundColor: c['background-subtle'], border: `1px solid ${c['border-divider']}`, padding: '1px 6px', borderRadius: 4, whiteSpace: 'nowrap' as const }}>
...
const QuestionRow: React.FC<{
  ...
  return (
    <div
      ...
      onMouseLeave={() => setHovered(false)}
    >
      <span style={{ fontSize: fs.xs, color: c['content-secondary'], flexShrink: 0, paddingTop: sp.A, minWidth: 18, textAlign: 'right' as const }}>
        {index + 1}.
      </span>
      <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ padding: `${sp.D}px 20px`, borderBottom: `1px solid ${c['border-divider']}`, flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ fontSize: fs.xs, color: c['content-secondary'], marginTop: sp.A, marginLeft: sp.E }}>
                  ...
                  readOnly
                    ? (
                      <div key={i} style={{ display: 'flex', gap: sp.C, alignItems: 'flex-start', padding: `${sp.B}px 0`, borderBottom: `1px solid ${c['border-divider']}` }}>
                        <span style={{ fontSize: fs.xs, color: c['content-secondary'], flexShrink: 0, paddingTop: sp.A, minWidth: 18, textAlign: 'right' as const }}>{i + 1}.</span>
