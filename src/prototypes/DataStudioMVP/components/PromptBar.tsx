...
const PromptBar = forwardRef<PromptBarRef, PromptBarProps>(({
  ...
  const border = focused || mentionActive
    ...
    : landingPage
      ? `1.5px solid ${c['border-subtle-hover']}`
      ...
      <div style={{ backgroundColor: c['background-base'], borderRadius: landingPage ? 14 : 12, border, boxShadow: landingPage ? '0 2px 12px rgba(25,35,49,0.07)' : '0px 0px 4px rgba(25,35,49,0.06), 0px 2px 4px rgba(25,35,49,0.04)', transition: 'border-color 0.15s' }}>
              ...
              {filtered.map((t, idx) => (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: sp.B }}>
                    <span style={{ fontSize: 12, color: c['content-secondary'], width: 12, textAlign: 'center' }}>{typeIcon(t.type)}</span>
        ...
        {attachedRefs.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: sp.A, padding: `0 ${sp.C}px ${sp.A}px` }}>
            {attachedRefs.map(r => (
              <span key={`${r.kind}-${r.label}`} style={{ display: 'inline-flex', alignItems: 'center', gap: sp.A, fontSize: fs.xs, backgroundColor: c['background-information'], color: c['content-brand'], padding: `2px ${sp.B}px`, borderRadius: 6, fontFamily: ff.mono }}>
        ...
        {attachedColumns.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: sp.A, padding: `0 ${sp.C}px ${sp.A}px` }}>
            {attachedColumns.map(name => (
              <span key={name} style={{ display: 'inline-flex', alignItems: 'center', gap: sp.A, fontSize: fs.xs, backgroundColor: c['background-sunken'], color: '#6D28D9', padding: `2px ${sp.B}px`, borderRadius: 6, fontFamily: ff.mono }}>
                ✦ {name}
        ...
        {errorChip && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: sp.A, padding: `0 ${sp.C}px ${sp.A}px` }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: sp.A, fontSize: fs.xs, backgroundColor: c['background-failure'], color: '#DC2626', border: '1px solid #FECACA', padding: `2px ${sp.B}px`, borderRadius: 6, fontFamily: ff.mono }}>
        ...
        {attachedTables.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: sp.A, padding: `0 ${sp.C}px ${sp.A}px` }}>
            {attachedTables.map(id => (
              <span key={id} style={{ display: 'inline-flex', alignItems: 'center', gap: sp.A, fontSize: fs.xs, backgroundColor: c['background-information'], color: c['content-brand'], padding: `2px ${sp.B}px`, borderRadius: 6, fontFamily: ff.mono }}>
                ⊞ {id}
