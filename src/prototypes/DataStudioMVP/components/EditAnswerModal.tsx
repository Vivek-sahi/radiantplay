...
export interface EditAnswerModalProps {
  ...
  /** Date columns, listed in their own picker section. */
  dates?: string[];
  /** Formula columns — the reference lists these under the dates. */
  formulas?: string[];
  /**
   * ── Query: the same editor, full screen ─────────────────────────────────────
   * Step 8 of the journey is *test the model before you save it*, and that test is a
   * Search data query. Vivek, 2026-08-19: replicate this surface full screen rather than
   * as the centred card the other two callers use.
   *
   * ⚠️ **Opt-in, so the readiness flow and the agent panel are untouched.** They open this
   * as "Edit Answer" over a dimmed page, and nothing about them changes.
   */
  fullScreen?: boolean;
  /** The header title. In full screen it matches the button that opened it — "Query". */
  heading?: string;
  onClose: () => void;
}
...
const EditAnswerModal: React.FC<EditAnswerModalProps> = ({
  title, dataset, tokens, measures, attributes, initialChecked = [], chart: chartData, filter,
  dates = [], formulas = [], fullScreen = false, heading = 'Edit Answer', onClose,
}) => {
  ...
  /*
    ── Surface treatment at page scale ─────────────────────────────────────────
    ⚠️ **The sunken body stays** — Vivek confirmed the grey behind the panels is right. What
    full screen drops is the *lift*: the card shadows and the search bar's elevation, which
    read as odd once the surface is a page rather than a dialog floating over one. Hairline
    borders on grey do the separating.

    And the header comes down. At 18px padding around 19px type it was a ~57px band, with a
    66px toolbar under it — 120px of chrome before any content.
  */
  const cardShadow = fullScreen ? 'none' : CARD_SHADOW;
  const searchBarSurface: React.CSSProperties = fullScreen
  ...
  return (
    <div
      style={fullScreen
        /* Full screen means the surface *is* the page: no dimmed ground behind it, because
           there is nothing behind it to look at. */
        ? { position: 'fixed', inset: 0, zIndex: 3000, background: '#fff', display: 'flex' }
        : { position: 'fixed', inset: 0, zIndex: 3000, background: 'rgba(25,35,49,0.45)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '40px 24px' }}
      onClick={fullScreen ? undefined : onClose}
    >
      <div
        onClick={e => e.stopPropagation()}
        style={fullScreen
          ? { width: '100%', height: '100%', background: '#fff', display: 'flex', flexDirection: 'column', overflow: 'hidden', fontFamily: ff.primary }
          : { width: '100%', maxWidth: 1280, height: 'calc(100vh - 80px)', background: '#fff', borderRadius: 12, boxShadow: '0 24px 64px rgba(25,35,49,0.28)', display: 'flex', flexDirection: 'column', overflow: 'hidden', fontFamily: ff.primary }}
      >
        {/* Header — divided from the toolbar below it. The title is the surface's, the
            toolbar belongs to the answer; without a rule they read as one tall band.

            ⚠️ **Full screen carries Close in the header**, because it has no dimmed
            backdrop to click and no footer: the way out has to be visible on the surface. */}
        <div style={{
          display: 'flex', alignItems: 'center', flexShrink: 0,
          height: fullScreen ? 48 : undefined,
          padding: fullScreen ? '0 20px' : '18px 24px',
          borderBottom: `1px solid ${c['border-divider']}`,
        }}>
          <span style={{ fontSize: fullScreen ? fs.lg : 19, fontWeight: fw.semibold, color: c['content-primary'], letterSpacing: '-0.2px' }}>{heading}</span>
          {fullScreen && (
            <button
              onClick={onClose}
              style={{ ...pillBtn, marginLeft: 'auto', height: 30, padding: '0 16px', border: 'none', background: c['background-subtle'], color: c['content-primary'] }}
            >Close</button>
          )}
        </div>
        {/* Search / token toolbar */}
        <div style={{ display: 'flex', alignItems: 'center', gap: sp.C, padding: fullScreen ? `${sp.B}px 20px` : '12px 24px', flexShrink: 0, background: '#fff' }}>
          ...
          <div style={{
            ...
            height: fullScreen ? 36 : 42,
            background: '#fff', borderRadius: 10, ...searchBarSurface,
          }}>
        ...
        <div style={{ flex: 1, display: 'flex', minHeight: 0, background: SUNKEN, padding: sp.C, gap: sp.C }}>
          {/* Column picker */}
          {panelOpen && (
          <aside style={{ width: 250, flexShrink: 0, display: 'flex', flexDirection: 'column', minHeight: 0, background: '#fff', border: `1px solid ${CARD_BORDER}`, borderRadius: 10, boxShadow: cardShadow, overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', paddingBottom: sp.C, borderTop: `1px solid ${c['border-divider']}` }}>
              ...
              {/* Dates and formulas are their own sections in the reference, not attributes:
                  a date is what you group *by time*, and a formula is a column you made. */}
              {dates.length > 0 && (
                <>
                  <div style={{ padding: '12px 12px 4px', fontSize: fs.sm, fontWeight: fw.regular, color: c['content-secondary'], display: 'flex', alignItems: 'center', gap: sp.B }}>
                    <svg width="10" height="10" viewBox="0 0 12 12" fill="none"><path d="M3 4.5l3 3 3-3" stroke="#8B96A5" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>Dates
                  </div>
                  {dates.map(checkRow)}
                </>
              )}
              <div style={{ padding: '12px 12px 4px', fontSize: fs.sm, fontWeight: fw.regular, color: c['content-secondary'], display: 'flex', alignItems: 'center', gap: sp.B }}>
                <svg width="10" height="10" viewBox="0 0 12 12" fill="none"><path d="M3 4.5l3 3 3-3" stroke="#8B96A5" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>Formulas
              </div>
              {formulas.length > 0
                ? formulas.map(checkRow)
                : (
                  <div style={{ padding: '4px 12px 2px', fontSize: fs.sm, color: c['content-tertiary'] }}>
                    Click +Add to create a formula
                  </div>
                )}
            </div>
          </aside>
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', background: '#fff', border: `1px solid ${CARD_BORDER}`, borderRadius: 10, boxShadow: cardShadow, overflow: 'hidden' }}>
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', padding: '18px 22px' }}>
            ...
            {/* The reference states the count under the answer — it is the first thing you
                check when you are testing whether a model answers a question. */}
            {!chart && (
              <div style={{ marginTop: sp.B, fontSize: fs.sm, color: c['content-tertiary'] }}>
                Showing {tableRows.length} of {tableRows.length} rows
              </div>
            )}
          </div>
        ...
        {/* Footer. ⚠️ **Not in full screen** — Query is a place you *look*, not a form you
            commit: there is nothing to discard and nothing to be done editing. Close is in
            the header instead. */}
        {!fullScreen && (
