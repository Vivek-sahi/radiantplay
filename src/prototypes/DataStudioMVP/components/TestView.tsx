...
const BORDER = `1px solid ${tok['border-divider']}`;
...
const TestView: React.FC<TestViewProps> = ({ tables, onFixWithAI }) => {
  ...
  const sectionHeader = (id: string, label: string, count: number) => (
    <button
      ...
      style={{ display: 'flex', alignItems: 'center', gap: sp.B, width: '100%', padding: '7px 12px', border: 'none', background: 'transparent', cursor: 'pointer', fontFamily: FONT, textAlign: 'left' }}
    >
      ...
      <span style={{ fontSize: 12, fontWeight: 700, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em', flex: 1 }}>{label}</span>
      <span style={{ fontSize: 12, color: tok['content-tertiary'], fontWeight: 600 }}>{count}</span>
    </button>
  );
  ...
  const colRow = (col: string, kind: string, table?: string) => (
    <button
      ...
      style={{ display: 'flex', alignItems: 'center', gap: sp.B, width: '100%', padding: '5px 12px 5px 27px', border: 'none', background: 'transparent', cursor: 'pointer', fontFamily: FONT, textAlign: 'left' }}
      onMouseEnter={e => { e.currentTarget.style.background = tok['background-sunken']; }}
      onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}
    >
      <span style={{ flexShrink: 0, display: 'flex' }}>{KIND_ICON[kind]}</span>
      <span style={{ fontSize: 12, color: tok['content-primary'], fontWeight: 500, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{col}</span>
      {table && <span style={{ fontSize: 12, color: tok['content-tertiary'], fontWeight: 500, flexShrink: 0 }}>{table}</span>}
    </button>
  );
  ...
  const renderChart = (data: { rows: (string | number)[][]; chartLabelIdx: number; chartValueIdx: number; chartValueLabel: string }) => {
    ...
    return (
      <div style={{ padding: '18px 20px 10px', display: 'flex', flexDirection: 'column', gap: sp.C }}>
        {data.rows.map((r, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: sp.C }}>
            ...
            <div style={{ flex: 1, height: 22, background: tok['background-sunken'], borderRadius: 4, overflow: 'hidden' }}>
            ...
            <span style={{ width: 76, fontSize: 12, color: tok['content-primary'], fontWeight: 600, flexShrink: 0 }}>{String(r[data.chartValueIdx])}</span>
          </div>
        ))}
        <div style={{ fontSize: 12, color: '#8B96A5', paddingLeft: 142 }}>{data.chartValueLabel}</div>
      </div>
    );
  };
  ...
  const statusIcon = (status: QuestionStatus) => {
    switch (status) {
      ...
      default:
        return <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="7" stroke={tok['content-tertiary']} strokeWidth="1.3" /></svg>;
    }
  };
  ...
  const renderFeedback = (key: 'exact' | 'spotter') => {
    ...
    const thumb = (v: 'up' | 'down', active: boolean) => (
      <button
        ...
        onMouseEnter={e => { if (!active) e.currentTarget.style.background = tok['background-subtle']; }}
    ...
    return (
      <div style={{ padding: '9px 14px', borderTop: BORDER }}>
        {diffCount > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: sp.C, marginBottom: sp.C, padding: '9px 11px', borderRadius: 8, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.25)' }}>
            ...
            <span style={{ flex: 1, fontSize: 12, color: '#8A6D1A', lineHeight: 1.5 }}>
            ...
            <button
              ...
              style={{ height: 28, padding: '0 12px', borderRadius: 6, border: 'none', background: '#2770EF', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: FONT, display: 'flex', alignItems: 'center', gap: sp.B, flexShrink: 0 }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: sp.B }}>
          <span style={{ fontSize: 12, color: '#8B96A5', marginRight: sp.A }}>Was this correct?</span>
          {thumb('up', f.verdict === 'up')}
          {thumb('down', f.verdict === 'down')}
          <button
            onClick={() => setVerdict(key, 'oos')}
            style={{ height: 26, padding: '0 11px', borderRadius: 99, cursor: 'pointer', fontFamily: FONT, fontSize: 12, fontWeight: 600, border: `1px solid ${oosActive ? tok['border-default'] : tok['border-subtle-hover']}`, background: oosActive ? tok['background-subtle'] : '#fff', color: oosActive ? '#4B5563' : '#8B96A5', transition: 'all 120ms' }}
          >
            Out of scope
          </button>
          <div style={{ flex: 1 }} />
          <button
            ...
            style={{ height: 26, padding: '0 11px', borderRadius: 6, cursor: 'pointer', fontFamily: FONT, fontSize: 12, fontWeight: 600, border: `1px solid ${tok['border-subtle-hover']}`, background: '#fff', color: '#4B5563', display: 'flex', alignItems: 'center', gap: sp.B, transition: 'border-color 120ms, background 120ms' }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = '#2770EF'; e.currentTarget.style.background = tok['background-sunken']; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = tok['border-subtle-hover']; e.currentTarget.style.background = '#fff'; }}
          >
        ...
        {f.verdict === 'down' && (
          <div style={{ marginTop: sp.C, paddingTop: sp.C, borderTop: `1px solid ${tok['border-divider']}` }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: tok['content-primary'], marginBottom: sp.B }}>What&rsquo;s wrong?</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: sp.B, marginBottom: sp.C }}>
              {FB_REASONS.map(r => {
                ...
                return (
                  <button
                    ...
                    style={{ padding: '4px 11px', borderRadius: 99, cursor: 'pointer', fontFamily: FONT, fontSize: 12, fontWeight: 500, whiteSpace: 'nowrap', border: `1px solid ${active ? '#2770EF' : tok['border-subtle-hover']}`, background: active ? '#2770EF' : '#fff', color: active ? '#fff' : '#4B5563', transition: 'all 120ms' }}
                  >
                    {r}
                  </button>
                );
            ...
            <button
              onClick={() => onFixWithAI?.(FIX_ITEMS[currentQ.id] ?? GENERIC_FIX)}
              disabled={!f.reason}
              style={{ height: 30, padding: '0 14px', borderRadius: 6, border: 'none', background: f.reason ? '#2770EF' : tok['background-subtle'], color: f.reason ? '#fff' : '#A5ACB9', fontSize: 12, fontWeight: 600, cursor: f.reason ? 'pointer' : 'default', fontFamily: FONT, display: 'flex', alignItems: 'center', gap: sp.B }}
  ...
  const tuneSurface = (
    <div style={{ flex: 1, display: 'flex', minHeight: 0, fontFamily: FONT }}>
      ...
      <div style={{ flex: 1, minWidth: 0, minHeight: 0, padding: sp.D, display: 'flex' }}>
        <div style={{ flex: 1, minWidth: 0, minHeight: 0, display: 'flex', flexDirection: 'column', background: '#fff', border: BORDER, borderRadius: 12, boxShadow: '0 1px 4px rgba(25,35,49,0.05)', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', padding: '11px 16px', borderBottom: BORDER, display: 'flex', alignItems: 'center', gap: sp.C, flexShrink: 0 }}>
            <span style={{ flexShrink: 0, display: 'flex' }}>{statusIcon(currentIdx < 0 ? 'untested' : currentQ.status)}</span>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: tok['content-tertiary'], textTransform: 'uppercase', letterSpacing: '0.07em', display: 'flex', alignItems: 'center', gap: sp.A }}>
                <svg width="10" height="10" viewBox="0 0 16 16" fill="none" style={{ color: tok['content-tertiary'] }}><circle cx="7" cy="7" r="5" stroke="currentColor" strokeWidth="1.6"/><path d="M11 11l3 3" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/></svg>
                Search data
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: sp.B, marginTop: sp.A }}>
                {currentIdx < 0 ? (
                  <button
                    ...
                    style={{ display: 'flex', alignItems: 'center', gap: sp.B, border: 'none', background: 'transparent', padding: 0, cursor: 'pointer', fontFamily: FONT, minWidth: 0, textAlign: 'left' }}
                  >
                  ...
                  <input
                    ...
                    style={{ flex: 1, minWidth: 0, border: 'none', outline: 'none', background: 'transparent', fontSize: 14.5, fontWeight: 600, color: tok['content-primary'], fontFamily: FONT }}
                  />
                ) : (
                  <button
                    onClick={() => setQuestionMenuOpen(o => !o)}
                    style={{ display: 'flex', alignItems: 'center', gap: sp.B, border: 'none', background: 'transparent', padding: 0, cursor: 'pointer', fontFamily: FONT, minWidth: 0, textAlign: 'left' }}
                  >
                    <span style={{ fontSize: 14.5, fontWeight: 600, color: tok['content-primary'], lineHeight: 1.35, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{currentQ.question}</span>
                  </button>
                )}
                {/* Switcher toggle */}
                <button
                  ...
                  style={{ width: 22, height: 22, flexShrink: 0, border: 'none', borderRadius: 5, background: questionMenuOpen ? tok['background-information'] : 'transparent', color: questionMenuOpen ? '#2770EF' : '#8B96A5', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}
                  onMouseEnter={e => { if (!questionMenuOpen) e.currentTarget.style.background = tok['background-subtle']; }}
            ...
            {currentIdx >= 0 && <span style={{ flexShrink: 0, fontSize: 12, fontWeight: 500, color: '#8B96A5' }}>{currentIdx + 1} of {questions.length}</span>}

            {/* Question dropdown */}
            {questionMenuOpen && (
              <>
                <div onClick={() => setQuestionMenuOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 40 }} />
                <div style={{ position: 'absolute', top: 'calc(100% - 4px)', left: 16, width: 400, maxWidth: 'calc(100% - 32px)', background: '#fff', border: `1px solid ${tok['border-subtle-hover']}`, borderRadius: 10, boxShadow: '0 10px 32px rgba(25,35,49,0.16)', zIndex: 50, overflow: 'hidden' }}>
                  <div style={{ padding: '11px 14px 9px', borderBottom: BORDER, display: 'flex', alignItems: 'baseline', gap: sp.B }}>
                    <span style={{ fontSize: 12.5, fontWeight: 700, color: tok['content-primary'] }}>Questions</span>
                    <span style={{ fontSize: 12, color: '#8B96A5' }}>{passingCount} of {questions.length} passing</span>
                  </div>
                  {staleCount > 0 && !stalePromptDismissed && (
                    <div style={{ padding: '8px 14px', borderBottom: BORDER, background: tok['background-sunken'], display: 'flex', alignItems: 'center', gap: sp.B }}>
                      <span style={{ flex: 1, fontSize: 12, color: '#8A6D1A' }}>Model changed — {staleCount} question{staleCount > 1 ? 's' : ''} may differ.</span>
                      <button
                        onClick={() => { setQuestions(prev => prev.map(q => q.status === 'stale' ? { ...q, status: 'passing' } : q)); setStalePromptDismissed(true); }}
                        style={{ height: 24, padding: '0 10px', border: 'none', borderRadius: 5, background: '#F59E0B', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: FONT, flexShrink: 0 }}
                      >
                        Re-check
                      </button>
                    </div>
                    ...
                    {questions.map((q, idx) => {
                      ...
                      return (
                        <button
                          ...
                          style={{ display: 'flex', alignItems: 'flex-start', gap: sp.B, width: '100%', padding: '9px 14px', border: 'none', background: isActive ? 'rgba(39,112,239,0.06)' : 'transparent', cursor: 'pointer', textAlign: 'left', fontFamily: FONT }}
                          onMouseEnter={e => { if (!isActive) e.currentTarget.style.background = tok['background-sunken']; }}
                        ...
                        >
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: 12.5, fontWeight: isActive ? 600 : 500, color: isActive ? '#1B58D4' : tok['content-primary'], lineHeight: 1.4, marginBottom: 1 }}>{q.question}</div>
                  ...
                  <button
                    ...
                    style={{ display: 'flex', alignItems: 'center', gap: sp.B, width: '100%', padding: '10px 14px', border: 'none', borderTop: BORDER, background: 'transparent', cursor: 'pointer', textAlign: 'left', fontFamily: FONT, color: '#2770EF', fontSize: 12.5, fontWeight: 600 }}
                    onMouseEnter={e => { e.currentTarget.style.background = tok['background-sunken']; }}
                    onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}
                  >
          ...
          <div style={{ flex: 1, minHeight: 0, display: 'flex' }}>
            {/* Model columns (kept) */}
            <aside style={{ width: 232, flexShrink: 0, background: tok['background-base'], borderRight: BORDER, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
              <div style={{ padding: '11px 12px 9px', borderBottom: BORDER, flexShrink: 0 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: tok['content-primary'], marginBottom: sp.B }}>Model columns</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: sp.B, height: 28, background: '#fff', border: BORDER, borderRadius: 6, padding: '0 10px' }}>
                  <svg width="11" height="11" viewBox="0 0 16 16" fill="none" style={{ color: tok['content-tertiary'], flexShrink: 0 }}><circle cx="7" cy="7" r="5" stroke="currentColor" strokeWidth="1.5"/><path d="M11 11l3 3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>
                  <input
                    value={colSearch}
                    onChange={e => setColSearch(e.target.value)}
                    placeholder="Find a column..."
                    style={{ border: 'none', background: 'transparent', fontSize: 12, color: tok['content-primary'], outline: 'none', flex: 1, minWidth: 0, fontFamily: FONT }}
                  />
                </div>
              </div>
          ...
          {/* Token query bar */}
          <div style={{ padding: '16px 20px', flexShrink: 0, background: tok['background-sunken'] }}>
            <div style={{
              display: 'flex', alignItems: 'center', gap: sp.B, minHeight: 40, background: '#fff',
              border: BORDER, borderRadius: 7, boxShadow: '0 1px 3px rgba(25,35,49,0.04)',
              padding: '5px 6px 5px 12px',
            }}>
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: sp.A, flexWrap: 'wrap', minWidth: 0 }}>
                {tokens.map(t => (
                  <span key={t} style={{ display: 'inline-flex', alignItems: 'center', gap: sp.A, fontSize: 12, fontWeight: 500, padding: '3px 8px', borderRadius: 5, background: tok['background-information'], color: '#1B58D4', border: '1px solid rgba(39,112,239,0.18)', whiteSpace: 'nowrap' }}>
                    {t}
                    <button
                ...
                <input
                  ...
                  style={{ flex: 1, minWidth: 100, border: 'none', background: 'transparent', outline: 'none', fontSize: 12, color: tok['content-primary'], fontFamily: FONT, padding: '3px 0' }}
                />
              </div>
              {tokens.length > 0 && (
                <button onClick={clearAll} title="Clear all" style={{ width: 24, height: 24, border: 'none', background: 'transparent', borderRadius: 5, color: tok['content-tertiary'], cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0, flexShrink: 0 }}>
              ...
              {isNLQuestion(draft) ? (
                <button
                  ...
                  disabled={loading}
                  style={{ height: 28, padding: '0 13px', borderRadius: 6, border: 'none', background: '#2770EF', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: FONT, flexShrink: 0, display: 'flex', alignItems: 'center', gap: sp.B }}
                  onMouseEnter={e => { e.currentTarget.style.background = '#2359B6'; }}
                  onMouseLeave={e => { e.currentTarget.style.background = '#2770EF'; }}
                >
                ...
                <button
                  ...
                  style={{
                    ...
                    background: !loading && tokens.length > 0 ? '#2770EF' : tok['background-subtle'],
            ...
            {(currentIdx < 0 && !exactAnswer && !loading) ? (
              <div style={{ display: 'flex', justifyContent: 'center', padding: '14px 6px 20px' }}>
                <div style={{ width: '100%', maxWidth: 580 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: sp.B, marginBottom: sp.D, background: '#fff', border: BORDER, borderRadius: 10, boxShadow: '0 1px 3px rgba(25,35,49,0.04)', padding: '6px 6px 6px 13px' }}>
                    ...
                    <input
                      ...
                      style={{ flex: 1, minWidth: 0, border: 'none', outline: 'none', background: 'transparent', fontSize: 13, color: tok['content-primary'], fontFamily: FONT }}
                    />
                    <button
                      onClick={askQuestion}
                      disabled={!askDraft.trim()}
                      style={{ height: 30, padding: '0 14px', borderRadius: 7, border: 'none', background: askDraft.trim() ? '#2770EF' : tok['background-subtle'], color: askDraft.trim() ? '#fff' : '#A5ACB9', fontSize: 12, fontWeight: 600, cursor: askDraft.trim() ? 'pointer' : 'default', fontFamily: FONT, flexShrink: 0 }}
                    >Ask</button>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: sp.C, marginBottom: sp.D }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 14.5, fontWeight: 700, color: tok['content-primary'], letterSpacing: '-0.2px' }}>Tune your model</div>
                      <div style={{ fontSize: 12, color: '#64748B', marginTop: 1 }}>Pick a question to test against the exact answer — or add your own.</div>
                    </div>
                    <span style={{ flexShrink: 0, fontSize: 12, fontWeight: 600, color: '#64748B', background: tok['background-subtle'], borderRadius: 99, padding: '3px 10px' }}>{passingCount}/{questions.length} passing</span>
                  </div>
                  {/* Question list */}
                  <div style={{ display: 'flex', flexDirection: 'column', gap: sp.B }}>
                    {questions.map((q, idx) => (
                      <button
                        ...
                        style={{ display: 'flex', alignItems: 'center', gap: sp.C, width: '100%', padding: '10px 13px', border: BORDER, borderRadius: 9, background: '#fff', cursor: 'pointer', textAlign: 'left', fontFamily: FONT, transition: 'border-color 130ms, box-shadow 130ms' }}
                        onMouseEnter={e => { e.currentTarget.style.borderColor = '#2770EF'; e.currentTarget.style.boxShadow = '0 2px 10px rgba(39,112,239,0.10)'; }}
                        onMouseLeave={e => { e.currentTarget.style.borderColor = tok['border-divider']; e.currentTarget.style.boxShadow = 'none'; }}
                      >
                        <span style={{ flexShrink: 0, display: 'flex' }}>{statusIcon(q.status)}</span>
                        <span style={{ flex: 1, minWidth: 0, fontSize: 12.5, fontWeight: 500, color: tok['content-primary'], lineHeight: 1.35, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{q.question}</span>
                        <svg width="11" height="11" viewBox="0 0 10 10" fill="none" style={{ color: tok['content-tertiary'], flexShrink: 0 }}><path d="M3.5 2l3 3-3 3" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>
                      </button>
                    ))}
                    <button
                      onClick={() => addQuestion()}
                      style={{ display: 'flex', alignItems: 'center', gap: sp.B, width: '100%', padding: '10px 13px', border: '1px dashed #C7CDD6', borderRadius: 9, background: 'transparent', cursor: 'pointer', textAlign: 'left', fontFamily: FONT, color: '#2770EF', fontSize: 12.5, fontWeight: 600, transition: 'border-color 130ms, background 130ms' }}
                      onMouseEnter={e => { e.currentTarget.style.borderColor = '#2770EF'; e.currentTarget.style.background = tok['background-sunken']; }}
                      onMouseLeave={e => { e.currentTarget.style.borderColor = tok['border-default']; e.currentTarget.style.background = 'transparent'; }}
                    >
              ...
              <div style={{ background: '#fff', border: BORDER, borderRadius: 9, boxShadow: '0 1px 3px rgba(25,35,49,0.04)', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: sp.B, padding: '12px 16px', borderBottom: BORDER }}>
                ...
                <div style={{ padding: '16px 16px 18px', display: 'flex', flexDirection: 'column', gap: sp.C }}>
                  {[240, 360, 320, 280].map((w, i) => (
                    <div key={i} style={{ height: 12, width: w, maxWidth: '85%', borderRadius: 4, background: tok['background-subtle'], animation: `tv-shimmer 1.1s ease-in-out ${i * 0.12}s infinite` }} />
                  ))}
                </div>
              </div>
            ) : exactAnswer ? (
              <>
                <div style={{ background: '#fff', border: BORDER, borderRadius: 9, boxShadow: '0 1px 3px rgba(25,35,49,0.04)', overflow: 'hidden' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: sp.C, padding: '11px 16px', borderBottom: BORDER, background: tok['background-sunken'] }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: '#8B96A5', textTransform: 'uppercase', letterSpacing: '0.06em', flexShrink: 0 }}>Exact answer</span>
                    <span style={{ flex: 1, fontSize: 12.5, fontWeight: 600, color: tok['content-primary'], minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{exactAnswer.title}</span>
                    <div style={{ display: 'flex', alignItems: 'center', background: '#fff', border: BORDER, borderRadius: 6, padding: 1, gap: 1, flexShrink: 0 }}>
                      {(['table', 'chart'] as const).map(m => (
                        <button
                          ...
                          style={{ fontSize: 12, fontWeight: 500, padding: '3px 10px', borderRadius: 5, border: 'none', cursor: 'pointer', fontFamily: FONT, background: resultMode === m ? '#2770EF' : 'transparent', color: resultMode === m ? '#fff' : '#8B96A5', transition: 'all 120ms' }}
                      ...
                      <thead>
                        <tr>
                          {exactAnswer.cols.map(c => (
                            <th key={c} style={{ textAlign: 'left', padding: '9px 16px', fontSize: 12, fontWeight: 700, color: '#8B96A5', textTransform: 'uppercase', letterSpacing: '0.04em', borderBottom: BORDER, background: tok['background-sunken'] }}>{c}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {exactAnswer.rows.map((r, i) => (
                          <tr key={i}>
                            {r.map((cell, j) => (
                              <td key={j} style={{ padding: '9px 16px', fontSize: 12.5, color: j === 0 ? tok['content-primary'] : '#4B5563', fontWeight: j === 0 ? 600 : 400, borderBottom: i < exactAnswer.rows.length - 1 ? `1px solid ${tok['border-divider']}` : 'none' }}>{String(cell)}</td>
                            ))}
                          </tr>
                        ))}
                      </tbody>
                ...
                {answerSource === 'question' && (!spotterShown ? (
                  <button
                    ...
                    style={{ width: '100%', marginTop: sp.C, height: 44, border: '1px dashed #C0C6CF', borderRadius: 9, background: '#fff', color: tok['content-primary'], fontSize: 12.5, fontWeight: 500, cursor: 'pointer', fontFamily: FONT, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: sp.B, transition: 'border-color 120ms, background 120ms' }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = '#2770EF'; e.currentTarget.style.background = tok['background-sunken']; }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = tok['border-default']; e.currentTarget.style.background = '#fff'; }}
                  >
                  ...
                  <div style={{ marginTop: sp.C, background: '#fff', border: BORDER, borderRadius: 9, boxShadow: '0 1px 3px rgba(25,35,49,0.04)', overflow: 'hidden' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: sp.C, padding: '11px 16px', borderBottom: BORDER, background: tok['background-sunken'] }}>
                      <span style={{ fontSize: 12, fontWeight: 700, color: '#7C3AED', textTransform: 'uppercase', letterSpacing: '0.06em', flexShrink: 0 }}>Spotter's answer</span>
                      <span style={{ flex: 1, fontSize: 12, color: '#64748B' }}>
                        {spotterLoading ? 'Running through Spotter…' : spotterAnswer ? spotterAnswer.reasoning : ''}
                      </span>
                    </div>
                    {spotterLoading ? (
                      <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: sp.C }}>
                        {[240, 320, 280].map((w, i) => (
                          <div key={i} style={{ height: 12, width: w, maxWidth: '80%', borderRadius: 4, background: tok['background-subtle'], animation: `tv-shimmer 1.1s ease-in-out ${i * 0.12}s infinite` }} />
                        ))}
                      </div>
                    ) : spotterAnswer ? (
                      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            ...
                            <tr key={i}>
                              {r.map((cell, j) => {
                                ...
                                return (
                                  <td
                                    key={j}
                                    style={{ padding: '9px 16px', fontSize: 12.5, color: j === 0 ? tok['content-primary'] : '#4B5563', fontWeight: j === 0 ? 600 : 400, borderBottom: i < spotterAnswer.rows.length - 1 ? `1px solid ${tok['border-divider']}` : 'none', background: isDifferent ? 'rgba(245,158,11,0.08)' : 'transparent', position: 'relative' }}
                                  >
                                    {String(cell)}
                                    {isDifferent && (
              ...
              <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 320 }}>
                <div style={{ textAlign: 'center', maxWidth: 360 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: tok['content-primary'], marginBottom: sp.B }}>Ready to test</div>
  ...
  const recBadge = (done: boolean) => (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: sp.A, fontSize: 12, fontWeight: 700, padding: '2px 7px', borderRadius: 99, letterSpacing: '0.03em', textTransform: 'uppercase', color: done ? '#06BF7F' : '#2770EF', background: done ? 'rgba(6,191,127,0.10)' : 'rgba(39,112,239,0.10)' }}>
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, background: tok['background-subtle'], fontFamily: FONT }}>
      ...
      {(() => {
        ...
        const sectionCard = (opts: {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', flex: opts.open ? 1 : '0 0 auto', minHeight: 0, background: '#fff', border: `1px solid ${tok['border-divider']}`, borderRadius: 14, boxShadow: CARD_SHADOW, overflow: 'hidden', transition: 'flex 180ms ease' }}>
            <button
              onClick={opts.onToggle}
              style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: sp.C, padding: '15px 17px', border: 'none', background: 'transparent', cursor: 'pointer', textAlign: 'left', fontFamily: FONT, borderBottom: opts.open ? `1px solid ${tok['border-divider']}` : 'none' }}
              onMouseEnter={e => { e.currentTarget.style.background = tok['background-sunken']; }}
              onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: sp.B }}>
                  <span style={{ fontSize: 14, fontWeight: 700, color: tok['content-primary'], letterSpacing: '-0.2px' }}>{opts.title}</span>
                  {opts.badge}
                </div>
                <div style={{ fontSize: 12, color: '#64748B', marginTop: sp.A }}>{opts.subtitle}</div>
              </div>
              <div style={{ width: 26, height: 26, borderRadius: 7, background: tok['background-subtle'], color: '#64748B', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        return (
          <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', gap: sp.D, padding: sp.D, overflow: 'hidden' }}>
            {sectionCard({
              open: evalOpen,
      ...
      {memoryModalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div style={{ width: 420, background: '#fff', borderRadius: 10, padding: sp.E, fontFamily: FONT }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: tok['content-primary'], marginBottom: sp.C }}>Add to Memory</div>
            <textarea
              value={memoryNote}
              onChange={e => setMemoryNote(e.target.value)}
              placeholder="Describe what to remember…"
              style={{ width: '100%', minHeight: 80, padding: sp.C, border: BORDER, borderRadius: 6, fontSize: 12, fontFamily: FONT, resize: 'vertical' }}
            />
            <div style={{ display: 'flex', gap: sp.B, marginTop: sp.C }}>
              <button onClick={() => setMemoryModalOpen(false)} style={{ flex: 1, height: 34, border: BORDER, borderRadius: 6, background: '#fff', color: tok['content-primary'], fontSize: 12.5, fontWeight: 500, cursor: 'pointer', fontFamily: FONT }}>Cancel</button>
      ...
      {notWhatIMeantOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div style={{ width: 420, background: '#fff', borderRadius: 10, padding: sp.E, fontFamily: FONT }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: tok['content-primary'], marginBottom: sp.B }}>Not what you meant?</div>
            <div style={{ fontSize: 12, color: '#64748B', marginBottom: sp.C }}>This is a semantic issue — the token mapping doesn't match your intent. Adjust the tokens above to fix.</div>
      ...
      {wrongAnswerOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div style={{ width: 420, background: '#fff', borderRadius: 10, padding: sp.E, fontFamily: FONT }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: tok['content-primary'], marginBottom: sp.C }}>What's wrong with the answer?</div>
            <textarea
            ...
            />
            <div style={{ display: 'flex', gap: sp.B, marginTop: sp.C }}>
              <button onClick={() => setWrongAnswerOpen(false)} style={{ flex: 1, height: 34, border: BORDER, borderRadius: 6, background: '#fff', color: tok['content-primary'], fontSize: 12.5, fontWeight: 500, cursor: 'pointer', fontFamily: FONT }}>Cancel</button>
