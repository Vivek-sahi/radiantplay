...
> ## ⚠️ POC V2 work lives in `POCV2_STATUS.md`, not here
> All current work is the **POC V2 / MVP** cut (`?v=pocv2`). Its work list, locked
> decisions, caching strategy and next-session plan are in **`POCV2_STATUS.md`** — read
> that file, not this one.
>
> Everything below is **demo-era** (Vision / POC / Demo, run-of-show). Kept for history.
> Demo is finished and not under review.
