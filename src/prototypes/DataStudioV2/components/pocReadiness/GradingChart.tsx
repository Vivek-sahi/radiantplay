...
const GradingChart: React.FC<{ chart?: QChart; height?: number }> = ({ chart, height = 176 }) => {
  const option = useMemo(() => {
    ...
    const catAxis = {
      ...
      axisTick: { show: false },
      axisLabel: { color: TICK, fontSize: 11 },
    };
    const valAxis = {
      type: 'value' as const,
      splitLine: { lineStyle: { color: GRID } },
      axisLabel: { color: '#9CA3AF', fontSize: 10, formatter: (v: number) => (unit === '$M' ? `${v}` : v >= 1000 ? `${v / 1000}k` : `${v}`) },
    };
    return {
      grid: { left: 6, right: 14, top: 14, bottom: series.length > 1 ? 30 : 6, containLabel: true },
      tooltip: { trigger: 'axis' as const, confine: true },
      legend: series.length > 1
        ? { bottom: 0, icon: 'roundRect', itemHeight: 8, itemWidth: 8, textStyle: { fontSize: 11, color: TICK } }
        : undefined,
