...
const TestFixCard: React.FC<{ items: TestFixItem[]; onAllFixed?: () => void }> = ({ items, onAllFixed }) => {
  ...
  return (
    <div style={{
      border: '1px solid #E2E6EC', borderRadius: 12, background: '#fff', overflow: 'hidden',
      ...
      {/* Header */}
      <div style={{
        padding: '13px 15px', borderBottom: '1px solid #EAEDF2',
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13.5, fontWeight: 700, color: '#1D232F', letterSpacing: '-0.2px' }}>
            {allDone ? 'All issues fixed' : `${items.length} recommended fix${items.length === 1 ? '' : 'es'}`}
          </div>
          <div style={{ fontSize: 12, color: '#64748B', lineHeight: 1.5, marginTop: 3 }}>
            {allDone
      ...
      <div>
        {items.map(item => {
          ...
          return (
            <div key={item.id} style={{ padding: '11px 14px', borderBottom: '1px solid #F6F8FA', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
              {/* Status dot / check */}
              <div style={{
                width: 18, height: 18, borderRadius: '50%', flexShrink: 0, marginTop: 1,
                border: st === 'done' ? 'none' : `1.5px solid ${st === 'fixing' ? '#2770EF' : '#D5DAE1'}`,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 3 }}>
                  <span style={{ fontSize: 12.5, fontWeight: 600, color: '#1D232F' }}>{item.title}</span>
                  <span style={{ fontSize: 8.5, fontWeight: 600, padding: '1px 5px', borderRadius: 4, color: lc.fg, background: lc.bg, textTransform: 'uppercase', letterSpacing: '0.05em', flexShrink: 0 }}>{item.layer}</span>
                </div>
                <div style={{ fontSize: 11.5, color: '#8B96A5', lineHeight: 1.5 }}>
              ...
              {st === 'pending' && (
                <button
                  ...
                  style={{ flexShrink: 0, height: 26, padding: '0 11px', borderRadius: 6, border: '1px solid #E2E6EC', background: '#fff', color: '#2770EF', fontSize: 11.5, fontWeight: 600, cursor: 'pointer', fontFamily: ff.primary, display: 'flex', alignItems: 'center', gap: 4 }}
              ...
              {st === 'fixing' && (
                <span style={{ flexShrink: 0, fontSize: 11, fontWeight: 600, color: '#2770EF', paddingTop: 5 }}>Fixing…</span>
              )}
            </div>
          );
        })}
      ...
      {!allDone && (
        <div style={{ padding: '10px 14px', display: 'flex', justifyContent: 'flex-end' }}>
          <button
            ...
            style={{ height: 30, padding: '0 15px', borderRadius: 7, border: 'none', background: pendingCount ? '#2770EF' : '#EAEDF2', color: pendingCount ? '#fff' : '#A5ACB9', fontSize: 12, fontWeight: 600, cursor: pendingCount ? 'pointer' : 'default', fontFamily: ff.primary, display: 'flex', alignItems: 'center', gap: 6 }}
