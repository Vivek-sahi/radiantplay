...
import PrepContextPanel, { RuleEntry } from './PrepContextPanel';
import RulesChainPanel from './RulesChainPanel';
...
type CenterView  = 'table' | 'rules-chain';
...
type ScriptStep =
  ...
  | { type: 'fix';       scope?: Array<keyof DataRow>; message?: string; rules?: RuleEntry[] };

// ── Rules chain entries per journey path ─────────────────────────────────────

const PROBLEM_RULES: RuleEntry[] = [
  { id: 'r1', column: 'billing_entry_aws', table: 'billing_accounts', issueType: 'Null values',       transformation: "COALESCE(billing_entry_aws, '')" },
  { id: 'r2', column: 'operation',         table: 'billing_accounts', issueType: 'Null values',       transformation: "COALESCE(operation, 'Unknown')" },
  { id: 'r3', column: 'blended_cost',      table: 'billing_accounts', issueType: 'Statistical anomaly', transformation: 'CASE WHEN blended_cost > 20000 THEN NULL ELSE blended_cost END' },
];

const SYSTEM_RULES: RuleEntry[] = [
  { id: 'r1',  column: 'account_name',       table: 'billing_accounts', issueType: 'Null values',   transformation: "COALESCE(NULLIF(account_name, ''), 'Unknown')" },
  { id: 'r2',  column: 'billing_entry_aws',  table: 'billing_accounts', issueType: 'Null values',   transformation: "COALESCE(billing_entry_aws, '')" },
  { id: 'r3',  column: 'billing_period_aws', table: 'billing_accounts', issueType: 'Date format',   transformation: "TO_DATE(billing_period_aws, 'YYYY-MM-DD')" },
  { id: 'r4',  column: 'reservation_arn',    table: 'billing_accounts', issueType: 'Null values',   transformation: "COALESCE(reservation_arn, '(none)')" },
  { id: 'r5',  column: 'operation',          table: 'billing_accounts', issueType: 'Null values',   transformation: "COALESCE(operation, 'Unknown')" },
  { id: 'r6',  column: 'line_item_type',     table: 'billing_accounts', issueType: 'Type mismatch', transformation: "CASE WHEN line_item_type ~ '^[0-9]+$' THEN 'Usage' ELSE line_item_type END" },
  { id: 'r7',  column: 'blended_cost',       table: 'billing_accounts', issueType: 'Statistical anomaly', transformation: 'CASE WHEN blended_cost > 20000 THEN NULL ELSE blended_cost END' },
  { id: 'r8',  column: 'usage_amount',       table: 'line_items',       issueType: 'Null values',   transformation: 'COALESCE(usage_amount, 0)' },
  { id: 'r9',  column: 'unit_price',         table: 'line_items',       issueType: 'Null values',   transformation: 'COALESCE(unit_price, 0)' },
  { id: 'r10', column: 'discount_pct',       table: 'line_items',       issueType: 'Null values',   transformation: 'COALESCE(discount_pct, 0)' },
  { id: 'r11', column: 'region',             table: 'line_items',       issueType: 'Blank strings', transformation: "NULLIF(TRIM(region), '')" },
  { id: 'r12', column: 'subscription_start', table: 'subscriptions',    issueType: 'Date format',   transformation: "TO_DATE(subscription_start, 'YYYY-MM-DD')" },
  { id: 'r13', column: 'subscription_end',   table: 'subscriptions',    issueType: 'Date format',   transformation: "TO_DATE(subscription_end, 'YYYY-MM-DD')" },
  { id: 'r14', column: 'plan_name',          table: 'subscriptions',    issueType: 'Blank strings', transformation: "NULLIF(TRIM(plan_name), '')" },
];

// ── Scripts ───────────────────────────────────────────────────────────────────

const PROBLEM_SCRIPT: ScriptStep[] = [
  { type: 'agent', content: "Tell me what's happening — which column or report is giving you trouble?", delay: 500 },
  { type: 'wait' },
  ...
  { type: 'wait', chips: ['Fix the anomalies'] },
  {
    type: 'agent',
...
const ScorePopover: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  return (
    <div style={{
      position: 'absolute', top: 44, right: 0, width: 320, zIndex: 500,
...
const renderContent = (text: string): React.ReactNode => {
  return text.split('\n').map((line, i) => {
    ...
    if (/^[A-Za-z].{0,40}:$/.test(line)) {
      return <div key={i} style={{ fontWeight: 600, color: '#0f172a', marginTop: 6, lineHeight: 1.6 }}>{line}</div>;
    }
...
const AgentPanelLeft: React.FC<AgentPanelProps> = ({
  phase, path, messages, inputValue, isLoading, chips, width,
  onInputChange, onSend, onChoosePath, onViewPlan, onCollapse,
}) => {
  ...
  const inputPlaceholder =
    phase === 'choosing' ? 'Type a question, or choose a workflow above…' : 'Or type your own response…';
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderBottom: `1px solid ${c['border-divider']}`,
        display: 'flex', alignItems: 'center', gap: sp.B, flexShrink: 0,
      }}>
        <div style={{
          width: 24, height: 24, borderRadius: 6,
          background: 'linear-gradient(135deg, #6366f1, #8b5cf6)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 12, color: '#fff', fontWeight: fw.semibold, flexShrink: 0,
        }}>S</div>
        <span style={{ fontSize: fs.md, fontWeight: fw.semibold, color: c['content-primary'], flex: 1 }}>SpotterPrep</span>
        {path && (
          <span style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: `${sp.D}px` }}>
          ...
          <div key={msg.id} style={{ marginBottom: sp.D }}>
            {msg.type === 'tool-call' ? (
              <div style={{
                ...
                color: c['content-secondary'], marginLeft: 30,
              }}>
              ...
              <div style={{ display: 'flex', gap: sp.C, alignItems: 'flex-start' }}>
                <div style={{
                  ...
                  background: 'linear-gradient(135deg, #6366f1, #8b5cf6)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 11, color: '#fff', fontWeight: fw.semibold, marginTop: 1,
                }}>S</div>
                <div style={{ flex: 1, minWidth: 0, fontSize: fs.sm, color: c['content-primary'] }}>
                  ...
                  {msg.type === 'view-plan' && (
                    <button
                      onClick={onViewPlan}
                      style={{
                        marginTop: sp.C, display: 'inline-block',
                        padding: `${sp.B}px ${sp.C}px`,
                        border: `1px solid ${c['border-default']}`,
                        borderRadius: 6, backgroundColor: c['background-base'],
                        color: c['content-brand'], fontSize: fs.sm,
                        fontWeight: fw.medium, cursor: 'pointer', fontFamily: ff.primary,
                      }}
                    >
                      Review plan →
                    </button>
              ...
              <div style={{ display: 'flex', gap: sp.C, alignItems: 'flex-start' }}>
                <div style={{
                  ...
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 10, color: '#fff', fontWeight: fw.semibold, marginTop: 1,
                }}>V</div>
            ...
            {chips.map(chip => (
              <button
                ...
                style={{
                  ...
                  backgroundColor: '#eef2ff', fontSize: 11, color: '#4338ca',
        ...
        {phase === 'fixing' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: sp.B, marginTop: sp.B }}>
            <div style={{ width: 14, height: 14, borderRadius: '50%', border: `2px solid ${c['border-divider']}`, borderTopColor: '#6366f1', animation: 'prep-spin 0.8s linear infinite', flexShrink: 0 }} />
        ...
        <style>{`@keyframes prep-spin { to { transform: rotate(360deg); } }`}</style>
        <div ref={chatEndRef} />
      </div>
        ...
        <div style={{ display: 'flex', gap: sp.B, alignItems: 'flex-end' }}>
          <textarea
            ...
            style={{
              ...
              overflow: 'auto', padding: `${sp.C}px ${sp.C}px`,
              border: `1px solid ${c['border-divider']}`,
...
const MODEL_NAME = 'fnops-final';
const MODEL_TABLES = ['billing_accounts', 'line_items', 'subscriptions'];

const PrepSessionV2: React.FC<PrepSessionV2Props> = ({ onExit, onPublish }) => {
  // ── Core state (same as PrepSession) ────────────────────────────────────────
  ...
  const [rulesChain, setRulesChain] = useState<RuleEntry[]>([]);
  ...
  const dragStartW    = useRef(AGENT_DEFAULT);
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      const next = Math.min(Math.max(dragStartW.current + delta, AGENT_MIN), window.innerWidth - 340);
      setAgentWidth(next);
    };
    const onUp = () => {
      setIsDragging(false);
  ...
  // ── Agent logic (same as PrepSession) ────────────────────────────────────
  ...
  const playScript = useCallback(async (from: number) => {
    ...
    while (i < steps.length) {
      ...
      if (step.type === 'fix') {
        ...
        await wait(2800);
        if (!isMounted.current) return;
        if (step.scope && step.scope.length > 0) setFixedCols(new Set(step.scope));
        if (step.rules) setRulesChain(step.rules);
        hasFixesRef.current = true;
        setHasFixes(true);
        setPhase('done');
        setCompareEnabled(true);
        setDataView('after');
        addMessage({ role: 'agent', type: 'fix-result', content: step.message ?? 'Done. Fixes applied. Click Save to persist the rules chain.' });
        return;
      }
  ...
  const handleSend = useCallback((text: string) => {
    ...
    playScript(scriptIndexRef.current);
  ...
  return (
    <div style={{
      ...
      fontFamily: ff.primary, backgroundColor: c['background-base'],
    }}>
      <style>{`@keyframes prep-slide-in { from { opacity: 0; transform: translateX(8px); } to { opacity: 1; transform: translateX(0); } }`}</style>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: sp.B, flex: 1, minWidth: 0 }}>
          <div style={{
            ...
            background: 'linear-gradient(135deg, #6366f1, #8b5cf6)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: 11, color: '#fff', fontWeight: fw.semibold, flexShrink: 0,
          }}>S</div>
          <span style={{ fontSize: fs.xs, color: c['content-secondary'] }}>SpotterPrep</span>
          <span style={{ fontSize: fs.xs, color: c['content-tertiary'] }}>·</span>
          <span style={{ fontSize: fs.sm, fontWeight: fw.semibold, color: c['content-primary'], overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{MODEL_NAME}</span>
        </div>
          ...
          <div ref={scoreRef} style={{ position: 'relative', flexShrink: 0 }}>
            <button
              ...
              style={{
                ...
                padding: `3px ${sp.C}px`, borderRadius: 6,
                backgroundColor: hasFixes ? '#f0fdf4' : '#fff7ed',
                border: `1px solid ${hasFixes ? '#a7f3d0' : '#fed7aa'}`,
                cursor: 'pointer', fontFamily: ff.primary,
              }}
            >
              <span style={{ fontSize: 12, fontWeight: fw.semibold, color: hasFixes ? '#059669' : '#c2410c' }}>
                {hasFixes ? QUALITY_SCORE.after.grade : before.grade}
              </span>
              <span style={{ fontSize: 11, fontWeight: fw.semibold, color: hasFixes ? '#059669' : '#c2410c' }}>
                {hasFixes ? QUALITY_SCORE.after.score : before.score}
              </span>
              {hasFixes && (
                <span style={{ fontSize: 10, color: '#059669', marginLeft: 2 }}>
                  ↑ +{QUALITY_SCORE.after.score - before.score}
                </span>
              )}
              <svg width="9" height="9" viewBox="0 0 10 10" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" style={{ color: hasFixes ? '#059669' : '#c2410c' }}>
        ...
        <button
          ...
          style={{
            ...
            backgroundColor: contextPanelOpen ? c['background-subtle'] : 'transparent',
            cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
            color: contextPanelOpen ? c['content-primary'] : c['content-secondary'], flexShrink: 0,
          }}
          onMouseEnter={e => { if (!contextPanelOpen) e.currentTarget.style.backgroundColor = c['background-subtle']; }}
          onMouseLeave={e => { if (!contextPanelOpen) e.currentTarget.style.backgroundColor = 'transparent'; }}
        >
        ...
        <button
          onClick={onExit}
          style={{
            height: 30, padding: `0 ${sp.C}px`, border: `1px solid ${c['border-default']}`,
        ...
        {/* Save */}
        <button
          onClick={hasFixes ? onPublish : undefined}
          style={{
            height: 30, padding: `0 ${sp.C}px`, border: 'none', borderRadius: 6,
            ...
            cursor: hasFixes ? 'pointer' : 'default',
            fontFamily: ff.primary, transition: 'background-color 0.15s', flexShrink: 0,
          }}
          onMouseEnter={e => { if (hasFixes) e.currentTarget.style.backgroundColor = '#1a5fd4'; }}
          onMouseLeave={e => { if (hasFixes) e.currentTarget.style.backgroundColor = '#2770EF'; }}
        >
          Save
        </button>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        {agentCollapsed ? (
          <div style={{
            flexShrink: 0, display: 'flex', alignItems: 'flex-start', paddingTop: sp.D,
            borderRight: `1px solid ${c['border-divider']}`,
          }}>
            <button
              ...
              style={{
                ...
                backgroundColor: 'transparent', cursor: 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                color: c['content-secondary'], margin: `0 ${sp.B}px`,
              }}
              onMouseEnter={e => (e.currentTarget.style.backgroundColor = c['background-subtle'])}
              onMouseLeave={e => (e.currentTarget.style.backgroundColor = 'transparent')}
            >
          ...
          <AgentPanelLeft
            phase={phase} path={path} messages={messages}
            inputValue={inputValue} isLoading={isLoading} chips={chips}
            width={agentWidth}
        ...
        <div style={{
          ...
          animation: 'prep-slide-in 0.4s ease-out both',
        }}>
          {centerView === 'rules-chain' ? (
            <RulesChainPanel
              rules={rulesChain}
            ...
            <div style={{
              ...
              <div style={{
                ...
                <svg width="16" height="16" viewBox="0 0 14 14" fill="none">
                  <rect x="1" y="1" width="12" height="12" rx="1.5" stroke={c['content-secondary']} strokeWidth="1.3" fill="none"/>
                  <line x1="1" y1="4.5" x2="13" y2="4.5" stroke={c['content-secondary']} strokeWidth="1.1"/>
                  <line x1="5" y1="4.5" x2="5" y2="13" stroke={c['content-secondary']} strokeWidth="1.1"/>
                </svg>
                <span style={{ fontSize: fs.sm, fontWeight: fw.medium, color: c['content-primary'], fontFamily: 'monospace', flex: 1 }}>billing_accounts</span>
                <span style={{ fontSize: 11, color: c['content-tertiary'] }}>68,432 rows · showing {RAW_ROWS.length} · 8 columns</span>
                {compareEnabled && (
                  <span style={{
                    fontSize: 10, fontWeight: fw.medium,
                    color: dataView === 'after' ? '#059669' : '#c2410c',
                    backgroundColor: dataView === 'after' ? '#dcfce7' : '#fff7ed',
                ...
                {compareEnabled && (
                  <button
                    ...
                    style={{
                      height: 28, padding: `0 ${sp.C}px`,
                      border: `1px solid ${c['border-default']}`, borderRadius: 5,
                      backgroundColor: 'transparent', color: c['content-brand'],
                      fontSize: 11, cursor: 'pointer', fontFamily: ff.primary,
                    }}
                    onMouseEnter={e => (e.currentTarget.style.backgroundColor = c['background-subtle'])}
                    onMouseLeave={e => (e.currentTarget.style.backgroundColor = 'transparent')}
                  >
              ...
              {/* Issue legend (before view only) */}
              {dataView === 'before' && (
                <div style={{
                  padding: `${sp.A}px ${sp.D}px`,
                  borderBottom: `1px solid ${c['border-divider']}`,
        ...
        {contextPanelOpen && (
          <PrepContextPanel
            ...
            modelName={MODEL_NAME}
            tables={MODEL_TABLES}
            onOpenRulesChain={() => { setCenterView('rules-chain'); }}
          />
        )}
