...
interface PrepContextPanelProps {
  rules: RuleEntry[];
  modelName: string;
  tables: string[];
  onOpenRulesChain: () => void;
}
...
const PrepContextPanel: React.FC<PrepContextPanelProps> = ({ rules, modelName, tables, onOpenRulesChain }) => {
  const [createdOpen, setCreatedOpen] = useState(true);
  const [contextOpen, setContextOpen] = useState(true);
  const [hoveredRulesChain, setHoveredRulesChain] = useState(false);
    ...
    <div style={{
      ...
      {createdOpen && (
        <div style={{ padding: `0 ${sp.C}px ${sp.C}px`, display: 'flex', flexDirection: 'column' }}>
          {!hasRules ? (
            ...
            <div
              onClick={onOpenRulesChain}
              onMouseEnter={() => setHoveredRulesChain(true)}
              onMouseLeave={() => setHoveredRulesChain(false)}
              style={{
                display: 'flex', alignItems: 'center', gap: 6,
                padding: `5px ${sp.B}px`, borderRadius: 4,
                cursor: 'pointer',
                backgroundColor: hoveredRulesChain ? c['background-subtle'] : 'transparent',
                transition: 'background-color 0.1s ease',
              }}
            >
              <DocIcon color={secondaryColor} />
              <span style={{ fontSize: fs.sm, color: c['content-primary'], flex: 1 }}>Rules chain</span>
              <span style={{ fontSize: 10, color: secondaryColor, flexShrink: 0 }}>{rules.length}</span>
            </div>
          )}
        </div>
      )}
      ...
      {contextOpen && (
        <div style={{ padding: `0 ${sp.C}px ${sp.C}px`, display: 'flex', flexDirection: 'column', gap: sp.C }}>
          ...
          {/* Model */}
          <div>
            <div style={{ fontSize: fs.xs, color: secondaryColor, fontWeight: fw.medium, padding: `0 ${sp.B}px`, marginBottom: 2 }}>Model</div>
            <div
              ...
              style={{
                ...
                backgroundColor: hoveredModel ? c['background-subtle'] : 'transparent',
                transition: 'background-color 0.1s ease',
              }}
            >
              <ModelIcon />
              <span style={{ flex: 1, fontSize: fs.sm, color: c['content-primary'] }}>{modelName}</span>
            </div>
          </div>
...
export default PrepContextPanel;
