...
import { ordersData, relationships, transformations } from '../data/mockData';
...
const CenterPanel: React.FC<CenterPanelProps> = ({ project }) => {
  return (
    <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', backgroundColor: c['background-sunken'] }}>
      {project.activeTab === 'visualizer' && <VisualizerView project={project} />}
...
const VisualizerView: React.FC<{ project: ProjectState }> = ({ project }) => {
  ...
  return (
    <div style={{ flex: 1, overflow: 'auto', padding: sp.H, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <svg width="620" height="420" viewBox="0 0 620 420" style={{ maxWidth: '100%' }}>
        ...
        <TableNode x={60} y={160} label="Orders" cols={8} calcBadge={hasCalc} healthyBadge={isHealthy} />

        {/* Campaigns & Users — only visible when tables added */}
        <TableNode x={360} y={60}  label="Campaigns" cols={9} />
        <TableNode x={360} y={290} label="Users"     cols={8} />

        {hasJoins && (
          <>
            {/* Orders → Campaigns (LEFT JOIN on campaign_id) */}
            <path d="M 240 195 C 295 195 295 100 360 100" stroke="#4A90E2" strokeWidth="1.5" fill="none" />
            <circle cx="240" cy="195" r="4" fill="#4A90E2" />
            <circle cx="360" cy="100" r="4" fill="#4A90E2" />
            <rect x="262" y="127" width="76" height="18" rx="4" fill="#EEF4FF" />
            <text x="300" y="140" fill="#4A90E2" fontSize="10" fontFamily="system-ui" textAnchor="middle" fontWeight="500">LEFT JOIN</text>
            <text x="300" y="122" fill="#94A3B8" fontSize="9" fontFamily="system-ui" textAnchor="middle">campaign_id</text>

            {/* Orders → Users (INNER JOIN on user_id) */}
            <path d="M 240 215 C 295 215 295 330 360 330" stroke="#4A90E2" strokeWidth="1.5" fill="none" />
            <circle cx="240" cy="215" r="4" fill="#4A90E2" />
            <circle cx="360" cy="330" r="4" fill="#4A90E2" />
            <rect x="262" y="255" width="82" height="18" rx="4" fill="#EEF4FF" />
            <text x="303" y="268" fill="#4A90E2" fontSize="10" fontFamily="system-ui" textAnchor="middle" fontWeight="500">INNER JOIN</text>
            <text x="303" y="250" fill="#94A3B8" fontSize="9" fontFamily="system-ui" textAnchor="middle">user_id</text>
          </>
        )}
      </svg>
    </div>
...
const TableNode: React.FC<{ x: number; y: number; label: string; cols: number; calcBadge?: boolean; healthyBadge?: boolean }> = ({ x, y, label, cols, calcBadge, healthyBadge }) => {
  const h = calcBadge ? 96 : 80;
  return (
    <g>
      <rect x={x} y={y} width={185} height={h} rx={8} fill="#fff" stroke={healthyBadge ? '#22C55E' : '#E2E8F0'} strokeWidth={healthyBadge ? 2 : 1.5} />
      <text x={x + 14} y={y + 24} fill="#1D232F" fontSize="13" fontWeight="600" fontFamily="system-ui">{label}</text>
      <text x={x + 14} y={y + 42} fill="#94A3B8" fontSize="11" fontFamily="system-ui">{cols} columns</text>
      {Array.from({ length: Math.min(cols, 5) }).map((_, i) => (
        <rect key={i} x={x + 14 + i * 22} y={y + 55} width={16} height={5} rx={3} fill="#E2E8F0" />
      ))}
      {calcBadge && (
        <text x={x + 14} y={y + 80} fill="#4A90E2" fontSize="10" fontFamily="system-ui">ƒx +3 calculated columns</text>
      )}
      {healthyBadge && (
        <text x={x + 130} y={y + 16} fill="#22C55E" fontSize="10" fontFamily="system-ui" fontWeight="600">✓ AI-ready</text>
      )}
    </g>
  );
};
...
const DataPreviewView: React.FC<{ project: ProjectState }> = ({ project }) => {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      {/* Toolbar */}
      <div style={{ height: 40, borderBottom: `1px solid ${c['border-divider']}`, backgroundColor: '#fff', display: 'flex', alignItems: 'center', padding: `0 ${sp.C}px`, gap: sp.B, flexShrink: 0 }}>
        {['←', '→', '↑↓', '⊞', '≡', '⇔', '$', '%', 'T', 'ƒx', '▼', '↓'].map(tool => (
          <button key={tool} style={{ background: 'none', border: 'none', cursor: 'pointer', color: c['content-secondary'], fontSize: 13, padding: '2px 4px', borderRadius: 3, fontFamily: 'inherit' }}
            onMouseEnter={e => e.currentTarget.style.backgroundColor = c['background-subtle']}
            onMouseLeave={e => e.currentTarget.style.backgroundColor = 'transparent'}
          >{tool}</button>
        ))}
        <div style={{ width: 1, height: 16, backgroundColor: c['border-divider'] }} />
        <div style={{ display: 'flex', alignItems: 'center', gap: sp.B, fontSize: 12, color: c['content-secondary'] }}>
          <span style={{ fontWeight: 500, color: c['content-primary'] }}>order_id</span>
          <span>ƒx</span>
        </div>
      </div>
        ...
        <table style={{ borderCollapse: 'collapse', fontSize: 12, width: 'max-content' }}>
          <thead>
            <tr style={{ backgroundColor: c['background-subtle'], position: 'sticky', top: 0, zIndex: 1 }}>
              <th style={{ width: 40, padding: `${sp.B}px ${sp.C}px`, borderBottom: `1px solid ${c['border-divider']}`, borderRight: `1px solid ${c['border-divider']}`, color: c['content-secondary'], fontWeight: 400, textAlign: 'center' }}>#</th>
              {extraCols.map(col => (
                <th key={col} style={{ padding: `${sp.B}px ${sp.D}px`, borderBottom: `1px solid ${c['border-divider']}`, borderRight: `1px solid ${c['border-divider']}`, color: c['content-secondary'], fontWeight: 500, textAlign: 'left', whiteSpace: 'nowrap', minWidth: col === 'return_on_spend' || col === 'campaign_performance' ? 160 : 120 }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    {col}
                    {(col === 'return_on_spend' || col === 'campaign_performance') && (
                      <span style={{ fontSize: 10, backgroundColor: '#EEF4FF', color: '#4A90E2', padding: '0 3px', borderRadius: 3 }}>ƒx</span>
                    )}
          ...
          <tbody>
            {displayRows.map((row, i) => (
              <tr key={row.order_id + i} style={{ backgroundColor: i % 2 === 0 ? '#fff' : '#FAFBFC' }}
              ...
              >
                ...
                {columns.map(col => {
                  ...
                  return (
                    <td key={col} style={{ padding: `${sp.A}px ${sp.D}px`, borderBottom: `1px solid ${c['border-divider']}`, borderRight: `1px solid ${c['border-divider']}`, whiteSpace: 'nowrap', color: isNull ? '#FDA4AF' : isAnomaly ? '#F97316' : isFlagged ? '#F97316' : c['content-primary'], backgroundColor: isAnomaly ? '#FFF7ED' : undefined }}>
                      {isNull ? <em style={{ color: '#FDA4AF' }}>null</em> : String(val)}
                      {isFlagged && <span style={{ marginLeft: 4, fontSize: 10, backgroundColor: '#FFF7ED', color: '#F97316', padding: '1px 4px', borderRadius: 3 }}>⚠</span>}
                    </td>
                  );
                })}
                {(project.buildStep === 'transformed' || project.buildStep === 'healthy') && (
                  <>
                    <td style={{ padding: `${sp.A}px ${sp.D}px`, borderBottom: `1px solid ${c['border-divider']}`, borderRight: `1px solid ${c['border-divider']}`, color: '#4A90E2' }}>
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: sp.D }}>
      ...
      <div style={{ marginBottom: sp.D }}>
        <button style={{ display: 'flex', alignItems: 'center', gap: sp.B, padding: `${sp.B}px ${sp.C}px`, border: `1px solid ${c['border-default']}`, borderRadius: 6, backgroundColor: '#fff', color: c['content-primary'], fontSize: 13, cursor: 'pointer' }}>
          <span>+</span> Add Cell <span style={{ fontSize: 10, color: c['content-secondary'] }}>▾</span>
        </button>
      </div>
...
const NotebookCell: React.FC<{ id: number; type: string; comment: string; query: string }> = ({ id, type, comment, query }) => (
  <div style={{ border: `1px solid ${c['border-divider']}`, borderRadius: 8, backgroundColor: '#fff', overflow: 'hidden' }}>
    {/* Cell header */}
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: `${sp.B}px ${sp.C}px`, borderBottom: `1px solid ${c['border-divider']}`, backgroundColor: c['background-subtle'] }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: sp.B }}>
        <span style={{ fontSize: 10, backgroundColor: type === 'sql' ? '#E0E7FF' : '#FEF9C3', color: type === 'sql' ? '#3730A3' : '#854D0E', padding: '1px 6px', borderRadius: 3, fontWeight: 600, textTransform: 'uppercase' }}>{type}</span>
        <span style={{ fontSize: 12, color: c['content-secondary'] }}>Cell {id}</span>
      </div>
    ...
    {/* Cell body */}
    <div style={{ padding: sp.C, fontFamily: 'Menlo, Monaco, monospace', fontSize: 12, lineHeight: 1.7 }}>
      <div style={{ color: '#94A3B8' }}>{comment}</div>
      {query.split('\n').map((line, i) => (
        <div key={i}>
          <span style={{ color: '#64748B', marginRight: 16, userSelect: 'none', fontSize: 11 }}>{i + 1}</span>
...
const ColorizedLine: React.FC<{ line: string; lang: string }> = ({ line, lang }) => {
      ...
      <span>
        {parts.map((part, i) =>
          keywords.test(part)
            ...
            : <span key={i} style={{ color: '#1E293B' }}>{part}</span>
        )}
      </span>
    );
  }
  if (lang === 'python') {
    ...
    return <span style={{ color: '#1E293B' }}>{line}</span>;
  }
  return <span style={{ color: '#1E293B' }}>{line}</span>;
};

const IconButton: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: c['content-secondary'], fontSize: 13, padding: '2px 4px', borderRadius: 3 }}
    onMouseEnter={e => e.currentTarget.style.backgroundColor = c['background-subtle']}
    onMouseLeave={e => e.currentTarget.style.backgroundColor = 'transparent'}
  >{children}</button>
);
...
const EmptyCenter: React.FC<{ icon: string; title: string; body: string }> = ({ icon, title, body }) => (
  <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    <div style={{ textAlign: 'center', maxWidth: 360, padding: sp.H }}>
      <div style={{ fontSize: 40, marginBottom: sp.D }}>{icon}</div>
      <h3 style={{ fontSize: 16, fontWeight: 600, color: c['content-primary'], margin: `0 0 ${sp.B}px` }}>{title}</h3>
      <p style={{ fontSize: 13, color: c['content-secondary'], lineHeight: 1.6, margin: 0 }}>{body}</p>
    </div>
  </div>
);
